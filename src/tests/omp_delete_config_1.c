/* Test 1 of OMP DELETE_CONFIG.
 * $Id$
 * Description: Test deleting a config that is referenced by a task.
 *
 * Authors:
...
main ()
{
  ...
  /* Create task that uses the config. */
  ...
  expected = add_entity (NULL, "delete_config_response", NULL);
