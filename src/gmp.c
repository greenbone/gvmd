 ...
 */
static gmp_parser_t *
gmp_parser_new (int (*write_to_client) (const char *, void *),
      ...
      while (index--)
        {
          create_report_result_t *result;
          result =
            (create_report_result_t *) g_ptr_array_index (data->results, index);
          if (result)
            {
              free (result->host);
...
typedef struct
{
  ...
  char *allow_simultaneous_ips; ///< Boolean. Whether to scan multiple IPs of a
                                ///< host simultaneously.
  char *asset_hosts_filter;     ///< Asset hosts.
  char *comment;                ///< Comment.
  char *exclude_hosts;          ///< Hosts to exclude from set.
  char *reverse_lookup_only;   ///< Boolean. Whether to consider only hosts that
                               ///< reverse lookup.
  char *reverse_lookup_unify;  ///< Boolean. Whether to unify based on reverse
                               ///< lookup.
  char *copy;                  ///< UUID of resource to copy.
...
typedef struct
{
  ...
  int
    family_selection_family_growing; ///< FAMILY_SELECTION/FAMILY growing flag.
 ...
 */
static create_user_data_t *create_user_data = &(command_data.create_user);
 ...
 */
static get_configs_data_t *get_configs_data = &(command_data.get_configs);

/**
 * @brief Parser callback data for GET_ALERTS.
 */
static get_alerts_data_t *get_alerts_data = &(command_data.get_alerts);

/**
 * @brief Parser callback data for GET_ASSETS.
 */
static get_assets_data_t *get_assets_data = &(command_data.get_assets);
 ...
 */
static get_feeds_data_t *get_feeds_data = &(command_data.get_feeds);

/**
 * @brief Parser callback data for GET_FILTERS.
 */
static get_filters_data_t *get_filters_data = &(command_data.get_filters);

/**
 * @brief Parser callback data for GET_GROUPS.
 */
static get_groups_data_t *get_groups_data = &(command_data.get_groups);

/**
 * @brief Parser callback data for GET_INFO.
 */
static get_info_data_t *get_info_data = &(command_data.get_info);

/**
 * @brief Parser callback data for GET_NOTES.
 */
static get_notes_data_t *get_notes_data = &(command_data.get_notes);

/**
 * @brief Parser callback data for GET_NVTS.
 */
static get_nvts_data_t *get_nvts_data = &(command_data.get_nvts);
 ...
 */
static get_overrides_data_t *get_overrides_data = &(command_data.get_overrides);
 ...
 */
static get_reports_data_t *get_reports_data = &(command_data.get_reports);
 ...
 */
static get_results_data_t *get_results_data = &(command_data.get_results);

/**
 * @brief Parser callback data for GET_ROLES.
 */
static get_roles_data_t *get_roles_data = &(command_data.get_roles);

/**
 * @brief Parser callback data for GET_scannerS.
 */
static get_scanners_data_t *get_scanners_data = &(command_data.get_scanners);

/**
 * @brief Parser callback data for GET_SCHEDULES.
 */
static get_schedules_data_t *get_schedules_data = &(command_data.get_schedules);

/**
 * @brief Parser callback data for GET_SETTINGS.
 */
static get_settings_data_t *get_settings_data = &(command_data.get_settings);
 ...
 */
static get_tags_data_t *get_tags_data = &(command_data.get_tags);

/**
 * @brief Parser callback data for GET_TARGETS.
 */
static get_targets_data_t *get_targets_data = &(command_data.get_targets);

/**
 * @brief Parser callback data for GET_TASKS.
 */
static get_tasks_data_t *get_tasks_data = &(command_data.get_tasks);

/**
 * @brief Parser callback data for GET_USERS.
 */
static get_users_data_t *get_users_data = &(command_data.get_users);

/**
 * @brief Parser callback data for GET_VULNS.
 */
static get_vulns_data_t *get_vulns_data = &(command_data.get_vulns);

/**
 * @brief Parser callback data for HELP.
 */
static help_data_t *help_data = &(command_data.help);

/**
 * @brief Parser callback data for MODIFY_CONFIG.
 */
static modify_config_data_t *modify_config_data = &(command_data.modify_config);

/**
 * @brief Parser callback data for MODIFY_ALERT.
 */
static modify_alert_data_t *modify_alert_data = &(command_data.modify_alert);

/**
 * @brief Parser callback data for MODIFY_ASSET.
 */
static modify_asset_data_t *modify_asset_data = &(command_data.modify_asset);

/**
 * @brief Parser callback data for MODIFY_AUTH.
 */
static modify_auth_data_t *modify_auth_data = &(command_data.modify_auth);
 ...
 */
static modify_filter_data_t *modify_filter_data = &(command_data.modify_filter);

/**
 * @brief Parser callback data for MODIFY_GROUP.
 */
static modify_group_data_t *modify_group_data = &(command_data.modify_group);
 ...
 */
static modify_role_data_t *modify_role_data = &(command_data.modify_role);
 ...
 */
static modify_target_data_t *modify_target_data = &(command_data.modify_target);

/**
 * @brief Parser callback data for MODIFY_TASK.
 */
static modify_task_data_t *modify_task_data = &(command_data.modify_task);
  ...
  if (gmp_parser->read_over)
    ...
    switch (client_state)
      ...
      case CLIENT_TOP:
        if (strcasecmp ("GET_VERSION", element_name) == 0)
        ...
        else
          {
            ...
            if (send_to_client (
                  XML_ERROR_SYNTAX ("gmp", "Only command GET_VERSION is"
      ...
      case CLIENT_AUTHENTIC:
        if (command_disabled (gmp_parser, element_name))
          {
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_UNAVAILABLE (
              "gmp", "Service unavailable: Command disabled"));
            g_set_error (error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
                         "Command Unavailable");
          }
        else if (strcasecmp ("AUTHENTICATE", element_name) == 0)
          ...
          {
            create_config_start (gmp_parser, attribute_names, attribute_values);
          ...
          {
            create_ticket_start (gmp_parser, attribute_names, attribute_values);
        ...
        else if (strcasecmp ("DELETE_CREDENTIAL", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "ultimate",
                                &attribute))
              delete_credential_data->ultimate = strcmp (attribute, "0");
            else
        ...
        else if (strcasecmp ("DELETE_REPORT_FORMAT", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "ultimate",
                                &attribute))
              delete_report_format_data->ultimate = strcmp (attribute, "0");
            else
        ...
        else if (strcasecmp ("DELETE_SCANNER", element_name) == 0)
          {
            ...
            append_attribute (attribute_names, attribute_values, "scanner_id",
                              &delete_scanner_data->scanner_id);
            if (find_attribute (attribute_names, attribute_values, "ultimate",
                                &attribute))
          ...
          {
            delete_start ("ticket", "Ticket", attribute_names,
                          attribute_values);
            set_client_state (CLIENT_DELETE_TICKET);
          }
        else if (strcasecmp ("DELETE_TLS_CERTIFICATE", element_name) == 0)
          {
            delete_start ("tls_certificate", "TLS Certificate", attribute_names,
                          attribute_values);
        ...
        else if (strcasecmp ("DELETE_USER", element_name) == 0)
          {
            ...
            append_attribute (attribute_names, attribute_values, "inheritor_id",
        ...
        else if (strcasecmp ("GET_AGGREGATES", element_name) == 0)
          {
            ...
            if (get_aggregates_data->type
              ...
              {
                append_attribute (attribute_names, attribute_values,
                                  "info_type", &get_aggregates_data->subtype);
              }

            append_attribute (attribute_names, attribute_values, "data_column",
                              &data_column);
            get_aggregates_data->data_columns =
              g_list_append (get_aggregates_data->data_columns, data_column);
            ...
            append_attribute (attribute_names, attribute_values, "sort_stat",
                              &(sort_data->stat));
            if (find_attribute (attribute_names, attribute_values, "sort_order",
                                &attribute))
              {
            ...
            if (strcmp (sort_data->field, "") || sort_order_given)
              {
                get_aggregates_data->sort_data =
                  g_list_append (get_aggregates_data->sort_data, sort_data);
              }
            ...
            if (find_attribute (attribute_names, attribute_values, "max_groups",
                                &attribute))
            ...
            get_data_parse_attributes (
              ...
              get_aggregates_data->type ? get_aggregates_data->type : "",
            ...
            if (find_attribute (attribute_names, attribute_values, "usage_type",
                                &attribute))
              {
                get_data_set_extra (&get_aggregates_data->get, "usage_type",
                                    attribute);
              }
        ...
        else if (strcasecmp ("GET_CONFIGS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_configs_data->get, "config",
                                       attribute_names, attribute_values);

            if (find_attribute (attribute_names, attribute_values, "tasks",
                                &attribute))
              get_configs_data->tasks = strcmp (attribute, "0");
            else
              get_configs_data->tasks = 0;

            if (find_attribute (attribute_names, attribute_values, "families",
                                &attribute))
            ...
            if (find_attribute (attribute_names, attribute_values, "usage_type",
                                &attribute))
              {
                get_data_set_extra (&get_configs_data->get, "usage_type",
                                    attribute);
              }
        ...
        else if (strcasecmp ("GET_ALERTS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_alerts_data->get, "alert",
                                       attribute_names, attribute_values);
            if (find_attribute (attribute_names, attribute_values, "tasks",
                                &attribute))
        ...
        else if (strcasecmp ("GET_ASSETS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_assets_data->get, "asset",
                                       attribute_names, attribute_values);
            if (find_attribute (attribute_names, attribute_values, "type",
                                &typebuf))
        ...
        else if (strcasecmp ("GET_CREDENTIALS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_credentials_data->get, "credential",
                                       attribute_names, attribute_values);

            if (find_attribute (attribute_names, attribute_values, "scanners",
                                &attribute))
              get_credentials_data->scanners = strcmp (attribute, "0");
            else
              get_credentials_data->scanners = 0;

            if (find_attribute (attribute_names, attribute_values, "targets",
                                &attribute))
        ...
        else if (strcasecmp ("GET_FILTERS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_filters_data->get, "filter",
                                       attribute_names, attribute_values);
            if (find_attribute (attribute_names, attribute_values, "alerts",
                                &attribute))
        ...
        else if (strcasecmp ("GET_NOTES", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "result",
                                &attribute))
        ...
        else if (strcasecmp ("GET_NVTS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "timeout",
                                &attribute))
            ...
            if (find_attribute (attribute_names, attribute_values, "sort_order",
                                &attribute))
              get_nvts_data->sort_order = strcmp (attribute, "descending");
            else
        ...
        else if (strcasecmp ("GET_PORT_LISTS", element_name) == 0)
          {
            ...
            get_data_parse_attributes (&get_port_lists_data->get, "port_list",
                                       attribute_names, attribute_values);
            if (find_attribute (attribute_names, attribute_values, "targets",
                                &attribute))
        ...
        else if (strcasecmp ("GET_REPORTS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "lean",
                                &attribute))
        ...
        else if (strcasecmp ("GET_REPORT_FORMATS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "params",
                                &attribute))
          ...
          {
            const gchar *attribute;
            get_data_parse_attributes (&get_results_data->get, "result",
                                       attribute_names, attribute_values);
            ...
            if (find_attribute (attribute_names, attribute_values, "get_counts",
                                &attribute))
        ...
        else if (strcasecmp ("GET_SETTINGS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "first",
                                &attribute))
            ...
            if (find_attribute (attribute_names, attribute_values, "max",
                                &attribute))
        ...
        else if (strcasecmp ("GET_TAGS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "names_only",
                                &attribute))
        ...
        else if (strcasecmp ("GET_SYSTEM_REPORTS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "brief",
                                &attribute))
        ...
        else if (strcasecmp ("GET_TASKS", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "usage_type",
                                &attribute))
              {
                get_data_set_extra (&get_tasks_data->get, "usage_type",
                                    attribute);
              }
        ...
        {
          append_attribute (attribute_names, attribute_values, "credential_id",
        ...
        {
          append_attribute (attribute_names, attribute_values, "permission_id",
        ...
        {
          modify_ticket_start (gmp_parser, attribute_names, attribute_values);
      ...
      case CLIENT_GET_AGGREGATES:
        if (strcasecmp ("DATA_COLUMN", element_name) == 0)
          {
            get_aggregates_data->data_columns =
              g_list_append (get_aggregates_data->data_columns, g_strdup (""));
        ...
        else if (strcasecmp ("SORT", element_name) == 0)
          {
            ...
            append_attribute (attribute_names, attribute_values, "stat",
                              &(sort_data->stat));
            if (find_attribute (attribute_names, attribute_values, "order",
                                &attribute))
              {
                sort_order_given = 1;
                sort_data->order = strcmp (attribute, "descending");
        ...
        else if (strcasecmp ("TEXT_COLUMN", element_name) == 0)
          {
            get_aggregates_data->text_columns =
              g_list_append (get_aggregates_data->text_columns, g_strdup (""));
      ...
      case CLIENT_MODIFY_CONFIG_FAMILY_SELECTION_FAMILY:
        if (strcasecmp ("ALL", element_name) == 0)
          set_client_state (CLIENT_MODIFY_CONFIG_FAMILY_SELECTION_FAMILY_ALL);
        else if (strcasecmp ("GROWING", element_name) == 0)
          set_client_state (
            CLIENT_MODIFY_CONFIG_FAMILY_SELECTION_FAMILY_GROWING);
        else if (strcasecmp ("NAME", element_name) == 0)
      ...
      case CLIENT_MODIFY_TASK:
        if (strcasecmp ("ALTERABLE", element_name) == 0)
        ...
        else if (strcasecmp ("FILE", element_name) == 0)
          {
            ...
            if (find_attribute (attribute_names, attribute_values, "action",
                                &attribute))
      ...
      case CLIENT_MODIFY_TICKET:
        modify_ticket_element_start (gmp_parser, element_name, attribute_names,
                                     attribute_values);
        break;

      case CLIENT_MODIFY_TLS_CERTIFICATE:
        modify_tls_certificate_element_start (
          gmp_parser, element_name, attribute_names, attribute_values);
        break;
      ...
      case CLIENT_MODIFY_USER:
        if (strcasecmp ("COMMENT", element_name) == 0)
          ...
          {
            const gchar *attribute;
            if (find_attribute (attribute_names, attribute_values, "allow",
                                &attribute))
          ...
          {
            const gchar *attribute;
            if (find_attribute (attribute_names, attribute_values, "modify",
                                &attribute))
      ...
      case CLIENT_CREATE_CONFIG:
        create_config_element_start (gmp_parser, element_name, attribute_names,
                                     attribute_values);
        break;
      ...
      case CLIENT_CREATE_REPORT_RR_ERRORS_ERROR:
        if (strcasecmp ("DESCRIPTION", element_name) == 0)
          set_client_state (CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_DESCRIPTION);
        else if (strcasecmp ("HOST", element_name) == 0)
          {
      ...
      case CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_HOST:
        if (strcasecmp ("ASSET", element_name) == 0)
          set_client_state (CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_HOST_ASSET);
        else if (strcasecmp ("HOSTNAME", element_name) == 0)
          set_client_state (CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_HOST_HOSTNAME);
        ELSE_READ_OVER;

      case CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_NVT:
        if (strcasecmp ("CVSS_BASE", element_name) == 0)
          set_client_state (CLIENT_CREATE_REPORT_RR_ERRORS_ERROR_NVT_CVSS_BASE);
        else if (strcasecmp ("NAME", element_name) == 0)
      ...
      case CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_HOST:
        if (strcasecmp ("ASSET", element_name) == 0)
          set_client_state (CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_HOST_ASSET);
        else if (strcasecmp ("HOSTNAME", element_name) == 0)
          set_client_state (
            CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_HOST_HOSTNAME);
        ELSE_READ_OVER;
      ...
      case CLIENT_CREATE_TICKET:
        create_ticket_element_start (gmp_parser, element_name, attribute_names,
                                     attribute_values);
        break;
          ...
          void *write_to_client_data)
{
  ...
  msg =
    get_nvt_xml (nvts, details, pref_count, preferences, timeout, config, 0);
  ...
  while (next (notes))
    {
      ...
      tag_count = resource_tag_count ("note", get_iterator_resource (notes), 1);

      if (count)
        (*count)++;

      if (note_iterator_task (notes))
        task_uuid (note_iterator_task (notes), &uuid_task);
      else
        uuid_task = NULL;

      if (note_iterator_result (notes))
        result_uuid (note_iterator_result (notes), &uuid_result);
      else
        uuid_result = NULL;
      ...
      if (/* The user is the owner. */
          (current_credentials.username && get_iterator_owner_name (notes)
        ...
        buffer_xml_append_printf (
          buffer, "<permission><name>Everything</name></permission>"
                  "</permissions>");
      else
        {
          iterator_t perms;
          ...
          while (next (&perms))
            buffer_xml_append_printf (
              buffer, "<permission><name>%s</name></permission>",
      ...
      if (include_notes_details == 0)
        {
          ...
          buffer_xml_append_printf (
            ...
            "<orphan>%i</orphan>",
            get_iterator_owner_name (notes) ? get_iterator_owner_name (notes)
                                            : "",
            note_iterator_nvt_oid (notes), note_iterator_nvt_name (notes),
            note_iterator_nvt_type (notes), get_iterator_creation_time (notes),
            get_iterator_modification_time (notes),
            note_iterator_active (notes), strlen (excerpt) < strlen (text),
            excerpt,
      ...
      else
        {
          ...
          buffer_xml_append_printf (
            ...
            note_iterator_hosts (notes) ? note_iterator_hosts (notes) : "",
            note_iterator_port (notes) ? note_iterator_port (notes) : "",
            note_iterator_severity (notes) ? note_iterator_severity (notes)
                                           : "",
            uuid_task ? uuid_task : "", name_task ? name_task : "", trash_task,
            ((note_iterator_task (notes) && (uuid_task == NULL))
             || (note_iterator_result (notes) && (uuid_result == NULL))));
          ...
          if (include_result && uuid_result && note_iterator_result (notes))
            {
              ...
              while (next (&results))
                buffer_results_xml (buffer, &results, 0, 0, /* Notes. */
                                    0,                      /* Note details. */
                                    0,                      /* Overrides. */
                                    0, /* Override details. */
                                    0, /* Tags. */
                                    0, /* Tag details. */
                                    0, /* Result details. */
                                    NULL, NULL, 0, -1, 0); /* Lean. */
              cleanup_iterator (&results);
            }
          else
            buffer_xml_append_printf (buffer, "<result id=\"%s\"/>",
          ...
          if (tag_count)
            {
              ...
              init_resource_tag_iterator (
                &tags, "note", get_iterator_resource (notes), 1, NULL, 1);
 ...
 * @param[in]  include_overrides_details  Whether to include details of
 * overrides.
 * @param[in]  include_result             Whether to include associated result.
  ...
  while (next (overrides))
    {
      ...
      char *uuid_task, *uuid_result;
      tag_count =
        resource_tag_count ("override", get_iterator_resource (overrides), 1);

      if (count)
        (*count)++;

      if (override_iterator_task (overrides))
        task_uuid (override_iterator_task (overrides), &uuid_task);
      else
        uuid_task = NULL;

      if (override_iterator_result (overrides))
        result_uuid (override_iterator_result (overrides), &uuid_result);
      else
        uuid_result = NULL;
      ...
      if (/* The user is the owner. */
          (current_credentials.username && get_iterator_owner_name (overrides)
      ...
      if (include_overrides_details == 0)
        {
          ...
          buffer_xml_append_printf (
            ...
            strlen (excerpt) < strlen (text), excerpt,
      ...
      else
        {
          ...
          if (include_result && uuid_result
              && override_iterator_result (overrides))
            {
              ...
              while (next (&results))
                buffer_results_xml (buffer, &results, 0, 0, /* Overrides. */
                                    0, /* Override details. */
                                    0, /* Overrides. */
                                    0, /* Override details. */
                                    0, /* Tags. */
          ...
          if (tag_count)
            {
              ...
              init_resource_tag_iterator (&tags, "override",
                                          get_iterator_resource (overrides), 1,
                                          NULL, 1);
                              ...
                              config_t config, int hide_passwords)
{
  ...
  buffer_xml_append_printf (buffer,
                            ...
                            "<name>%s</name>"
                            "<type>%s</type>",
                            oid ? oid : "", nvt ? nvt : "", id ? id : "",
                            real_name ? real_name : "",
                            real_name ? real_name : "", type ? type : "");
  ...
  else if (default_value && type && (strcmp (type, "password") == 0))
    buffer_xml_append_printf (buffer, "<default></default>");
  else
    buffer_xml_append_printf (buffer, "<default>%s</default>",
...
strdiff (const gchar *one, const gchar *two)
{
  ...
  cmd[6] = NULL;
  g_debug ("%s: Spawning in %s: %s \"%s\" \"%s\"", __func__, dir, cmd[0],
           cmd[1], cmd[2]);
  if ((g_spawn_sync (dir, cmd, NULL,            /* Environment. */
      ...
      || (WIFEXITED (exit_status) == 0) || WEXITSTATUS (exit_status))
    {
      if (WEXITSTATUS (exit_status) == 1)
        ...
        {
          g_debug ("%s: failed to run diff: %d (WIF %i, WEX %i)", __func__,
  ...
  if (task)
    {
      ...
      init_note_iterator (&notes, &get, 0, result, task);

      temp_buffer = g_string_new ("");
      buffer_notes_xml (temp_buffer, &notes, include_notes_details, 0, NULL);
  ...
  if (task)
    {
      ...
      init_override_iterator (&overrides, &get, 0, result, task);

      temp_buffer = g_string_new ("");
      buffer_overrides_xml (temp_buffer, &overrides, include_overrides_details,
                            0, NULL);
      if (lean == 0 || strlen (temp_buffer->str))
        {
      ...
      if (g_str_has_prefix (oid, "CVE-"))
        {
          ...
          buffer_xml_append_printf (buffer,
                                    ...
                                    "<cve>%s</cve>"
                                    "</nvt>",
                                    oid, oid, cvss_base, score,
                                    result_iterator_port (results), oid);
          g_free (cvss_base);
          return;
        }
      ...
      if (g_str_has_prefix (oid, "oval:"))
        {
          ...
          buffer_xml_append_printf (
            ...
            "</severities>"
            "<tags>summary=%s</tags>",
            oid, ovaldef_info_iterator_title (&iterator),
          ...
          while (*item)
            {
              ...
              buffer_xml_append_printf (buffer, "<ref type=\"cve\" id=\"%s\"/>",
                                        id);
      ...
      else
        {
          ...
          buffer_xml_append_printf (
            ...
            result_iterator_nvt_family (results) ?: "", cvss_base ?: "",
 ...
 * @param[in]  cert_loaded            Whether the CERT db is loaded.  0 not
 * loaded, -1 needs to be checked, else loaded.
 * @param[in]  lean                   Whether to include less info.
 */
void
buffer_results_xml (GString *buffer, iterator_t *results, task_t task,
                    ...
                    int include_details, const char *delta_state,
                    iterator_t *delta_results, int changed, int cert_loaded,
                    int lean)
{
  ...
  if (descr)
    {
      ...
      nl_descr_escaped = xml_escape_text_truncated (
        nl_descr, TRUNCATE_TEXT_LENGTH, TRUNCATE_TEXT_SUFFIX);
    }
  else
    {
      nl_descr = NULL;
  ...
  if (name)
    buffer_xml_append_printf (buffer, "<name>%s</name>", name);
  ...
  if (lean == 0)
    {
      ...
      if (owner_name)
        buffer_xml_append_printf (buffer, "<owner><name>%s</name></owner>",
                                  owner_name);
  ...
  if (comment && (lean == 0 || strlen (comment)))
    buffer_xml_append_printf (buffer, "<comment>%s</comment>", comment);

  creation_time = get_iterator_creation_time (results);
  if (creation_time)
    buffer_xml_append_printf (buffer, "<creation_time>%s</creation_time>",
                              creation_time);
  ...
  if (include_details)
    {
      ...
      buffer_xml_append_printf (buffer,
                                ...
                                result_report_id, result_task_id,
                                result_task_name);
  ...
  if (result_detection_reference (result, result_iterator_report (results),
      ...
      == 0)
    {
      ...
      buffer_xml_append_printf (buffer, "</details>"
                                        "</result>"
                                        "</detection>");
    }
  g_free (detect_ref);
  ...
  if (asset_id && strlen (asset_id))
    buffer_xml_append_printf (buffer, "<asset asset_id=\"%s\"/>", asset_id);
  else if (lean == 0)
    buffer_xml_append_printf (buffer, "<asset asset_id=\"\"/>");
  ...
  buffer_xml_append_printf (buffer, "<port>%s</port>", port);

  if (path && strcmp (path, ""))
    buffer_xml_append_printf (buffer, "<path>%s</path>", path);
  ...
  buffer_xml_append_printf (buffer,
                            ...
                            result_iterator_score (results), qod ? qod : "");
  ...
  if (include_notes && result_iterator_may_have_notes (results))
    buffer_result_notes_xml (buffer, result, selected_task,
                             include_notes_details, lean);

  if (include_overrides && result_iterator_may_have_overrides (results))
    buffer_result_overrides_xml (buffer, result, selected_task,
                                 include_overrides_details, lean);
      ...
      if (changed && delta_results)
        {
          ...
          delta_nl_descr =
            delta_descr ? convert_to_newlines (delta_descr) : NULL;
          diff =
            strdiff (descr ? nl_descr : "", delta_descr ? delta_nl_descr : "");
          ...
          if (diff)
            {
              ...
              if (split[0] && split[1] && split[2])
                diff_xml = xml_escape_text_truncated (
                  split[2], TRUNCATE_TEXT_LENGTH, TRUNCATE_TEXT_SUFFIX);
              else
                diff_xml = xml_escape_text_truncated (
                  diff, TRUNCATE_TEXT_LENGTH, TRUNCATE_TEXT_SUFFIX);
              g_strfreev (split);
      ...
      if (delta_results)
        {
          if (include_notes)
            buffer_result_notes_xml (
              buffer, result_iterator_result (delta_results), selected_task,
              include_notes_details, lean);
 ...
 */
static void
init_aggregate_lists (const gchar *group_column, const gchar *subgroup_column,
                      GList *data_column_list, GList *text_column_list,
                      GList *sort_data_list, gchar **group_column_type,
                      gchar **subgroup_column_type, GArray **data_column_types,
                      GArray **data_columns, GArray **text_column_types,
      ...
      if (strcmp (data_column, ""))
        {
          ...
          double c_sum = 0.0;
          g_array_append_val (*data_columns, current_column);
...
copy_word_counts_asc (gpointer key, gpointer value, gpointer data)
{
  ...
  g_sequence_insert_sorted ((GSequence *) data, new_count,
                            compare_count_data_reverse, NULL);
...
copy_word_counts_desc (gpointer key, gpointer value, gpointer data)
{
  ...
  g_sequence_insert_sorted ((GSequence *) data, new_count, compare_count_data,
                            NULL);
 ...
 */
static void
buffer_aggregate_wc_xml (GString *xml, iterator_t *aggregate, const gchar *type,
                         const char *group_column, GArray *sort_data,
                         int first_group, int max_groups)
{
  ...
  ignore_words =
    g_tree_new_full ((GCompareDataFunc) g_ascii_strcasecmp, NULL, g_free, NULL);
  ...
  g_string_append_printf (xml, "<group_column>%s</group_column>", group_column);
      ...
      while (split_string[current_index])
        {
          ...
          if (strlen (word) >= 3 && g_regex_match (word_regex, word, 0, NULL)
  ...
  if (sort_stat && strcasecmp (sort_stat, "count") == 0)
    {
      ...
      g_tree_foreach (word_counts,
                      sort_order ? copy_word_counts_asc : copy_word_counts_desc,
                      word_counts_sorted);
 ...
 */
static gboolean
buffer_aggregate_subgroup_value (gchar *key, long int *value, GString *buffer)
{
...
buffer_aggregate_xml (GString *xml, iterator_t *aggregate, const gchar *type,
                      ...
                      const char *subgroup_column_type, GArray *data_columns,
                      GArray *data_column_types, GArray *text_columns,
                      GArray *text_column_types, GArray *c_sums)
{
  int index;
  ...
  for (index = 0; index < data_columns->len; index++)
    {
      ...
      if (column_name && strcmp (column_name, ""))
        {
          g_string_append_printf (xml, "<data_column>%s</data_column>",
                                  column_name);
        }
    }
  ...
  for (index = 0; index < text_columns->len; index++)
    {
      ...
      if (column_name && strcmp (column_name, ""))
        {
          g_string_append_printf (xml, "<text_column>%s</text_column>",
                                  column_name);
        }
    }

  if (group_column)
    g_string_append_printf (xml, "<group_column>%s</group_column>",
                            group_column);

  if (subgroup_column)
    g_string_append_printf (xml, "<subgroup_column>%s</subgroup_column>",
                            subgroup_column);
  ...
  if (subgroup_column)
    {
      ...
      for (index = 0; index < data_columns->len; index++)
        {
          g_array_index (group_c_sums, GTree *, index) = g_tree_new_full (
            (GCompareDataFunc) g_strcmp0, NULL, g_free, g_free);
        }

      subgroup_c_counts =
        g_tree_new_full ((GCompareDataFunc) g_strcmp0, NULL, g_free, g_free);
    }
  else
    {
      group_mins = NULL;
          ...
          if (subgroup_column)
            {
              ...
              if (subgroup_c_count == NULL)
                {
                  ...
                  g_tree_insert (subgroup_c_counts, g_strdup (subgroup_value),
                                 subgroup_c_count);
                }
              *subgroup_c_count += aggregate_iterator_count (aggregate);
                  ...
                  for (index = 0; index < data_columns->len; index++)
                    {
                      ...
                      if (column_is_timestamp (data_column))
                        ...
                        g_string_append_printf (
                          ...
                          (g_array_index (group_mean_sums, double, index)
                           / aggregate_group_count),
                          g_array_index (group_sums, double, index), c_sum);
                    }
              ...
              g_string_append_printf (
                ...
                "<count>%d</count>"
                "<c_count>%ld</c_count>",
                subgroup_value_escaped ? subgroup_value_escaped : "",
                aggregate_iterator_count (aggregate), *subgroup_c_count);
            }
          else
            {
              // No subgrouping
      ...
      for (index = 0; index < data_columns->len; index++)
        {
              ...
              if (subgroup_c_sum == NULL)
                {
                  ...
                  *subgroup_c_sum = 0;
                  ...
                  g_tree_insert (c_sum_tree, g_strdup (subgroup_value),
                                 subgroup_c_sum);
                }
          ...
          if (column_is_timestamp (data_column))
            ...
            g_string_append_printf (xml,
                                    ...
                                    aggregate_iterator_mean (aggregate, index),
                                    aggregate_iterator_sum (aggregate, index),
                                    subgroup_column ? *subgroup_c_sum : c_sum);
        }

      for (index = 0; index < text_columns->len; index++)
        {
          const char *text =
            aggregate_iterator_text (aggregate, index, data_columns->len);
          gchar *text_escaped;
          ...
          g_string_append_printf (xml, "<text column=\"%s\">%s</text>",
                                  text_column,
      ...
      if (has_groups)
        {
          g_string_append_printf (xml,
                                  ...
                                  "<c_count>%ld</c_count>"
                                  "</group>",
                                  aggregate_group_count, previous_c_count);
        }

      // Also add overview of all subgroup values
      g_string_append_printf (xml, "<subgroups>");

      g_tree_foreach (subgroup_c_counts,
                      (GTraverseFunc) buffer_aggregate_subgroup_value, xml);
  ...
  if (group_column)
    {
      g_string_append_printf (xml,
                              ...
                              "</aggregate_column>",
                              type, group_column, group_column_type);
    }
  ...
  if (subgroup_column)
    {
      g_string_append_printf (xml,
                              ...
                              "<data_type>%s</data_type>"
                              "</aggregate_column>",
                              type, subgroup_column, subgroup_column_type);
    }
  ...
  for (index = 0; index < data_columns->len; index++)
    {
      ...
      g_string_append_printf (xml,
                              ...
                              "<data_type>%s</data_type>"
                              "</aggregate_column>",
                              column_name, type, column_name, column_type);
      g_string_append_printf (xml,
                              "<aggregate_column>"
                              "<name>%s_max</name>"
                              "<stat>max</stat>"
      ...
      switch (delete_##type (delete_##type##_data->type##_id,              \
        ...
        case 1:                                                            \
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (                       \
            "delete_" G_STRINGIFY (type), capital " is in use"));          \
          log_event_fail (G_STRINGIFY (type), capital,                     \
                          delete_##type##_data->type##_id, "deleted");     \
          break;                                                           \
        case 2:                                                            \
          if (send_find_error_to_client (                                  \
                "delete_" G_STRINGIFY (type), G_STRINGIFY (type),          \
                delete_##type##_data->type##_id, gmp_parser))              \
            {                                                              \
              error_send_to_client (error);                                \
              return;                                                      \
            }                                                              \
        ...
        case 3:                                                            \
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (                       \
            "delete_" G_STRINGIFY (type), "Attempt to delete a predefined" \
                                          " " G_STRINGIFY (type)));        \
          break;                                                           \
        case 99:                                                           \
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (                       \
            "delete_" G_STRINGIFY (type), "Permission denied"));           \
          log_event_fail (G_STRINGIFY (type), capital,                     \
                          delete_##type##_data->type##_id, "deleted");     \
          break;                                                           \
        default:                                                           \
...
handle_get_aggregates (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (type == NULL)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "A 'type' attribute is required"));
      return;
    }

  get = &get_aggregates_data->get;

  ret = init_get ("get_aggregates", &get_aggregates_data->get, "Aggregates",
                  &first);
  if (ret)
    {
      switch (ret)
        ...
        case 99:
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("get_aggregates", "Permission denied"));
          break;
        default:
  ...
  init_aggregate_lists (
    group_column, subgroup_column, get_aggregates_data->data_columns,
    get_aggregates_data->text_columns, get_aggregates_data->sort_data,
    &group_column_type, &subgroup_column_type, &data_column_types,
    &data_columns, &text_column_types, &text_columns, &sort_data, &c_sums);

  if (get_aggregates_data->mode
      && strcasecmp (get_aggregates_data->mode, "word_counts") == 0)
    {
      ret = init_aggregate_iterator (
        &aggregate, type, get, 0 /* distinct */, data_columns, group_column,
        subgroup_column, text_columns, NULL, /* ignore sorting */
        0,                                   /* get all groups */
        -1, NULL /* extra_tables */, NULL /* extra_where */);
    }
  else
    {
      ret = init_aggregate_iterator (
        &aggregate, type, get, 0 /* distinct */, data_columns, group_column,
        subgroup_column, text_columns, sort_data,
        get_aggregates_data->first_group, get_aggregates_data->max_groups,
        NULL /* extra_tables */, NULL /* extra_where */);
    }
  ...
  switch (ret)
    ...
    case 1:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Failed to find resource"));
      break;
    case 2:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Failed to find filter"));
      break;
    case 3:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Invalid data_column"));
      break;
    case 4:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Invalid group_column"));
      break;
    case 5:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Invalid resource type"));
      break;
    case 6:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "get_aggregates", "Trashcan not used by resource type"));
      break;
    case 7:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Invalid text_column"));
      break;
    case 8:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Invalid subgroup_column"));
      break;
    case 99:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_aggregates", "Permission denied"));
    ...
    default:
      ...
      /* fallthrough */
    case -1:
      SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_aggregates"));
      break;
    }
  ...
  if (get_aggregates_data->mode
      && strcasecmp (get_aggregates_data->mode, "word_counts") == 0)
    {
      buffer_aggregate_wc_xml (xml, &aggregate, type, group_column, sort_data,
  ...
  else
    {
      buffer_aggregate_xml (
        xml, &aggregate, type, group_column, group_column_type, subgroup_column,
        subgroup_column_type, data_columns, data_column_types, text_columns,
        text_column_types, c_sums);
    }
  ...
  manage_filter_controls (filter ? filter : get->filter, &first, NULL,
...
handle_get_alerts (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_alerts", "alert",
                                         get_alerts_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
  ...
  while (1)
    {
      ...
      ret = get_next (&alerts, &get_alerts_data->get, &first, &count,
                      init_alert_iterator);
      if (ret == 1)
        break;
      if (ret == -1)
      ...
      SEND_GET_COMMON (alert, &get_alerts_data->get, &alerts);
      ...
      SENDF_TO_CLIENT_OR_FAIL (
        "<condition>%s",
        alert_condition_name (alert_iterator_condition (&alerts)));
      init_alert_data_iterator (&data, get_iterator_resource (&alerts),
                                get_alerts_data->get.trash, "condition");
      while (next (&data))
      ...
      while (next (&data))
        {
          if (strcmp (alert_data_iterator_name (&data), "secinfo_type") == 0)
            has_secinfo_type = 1;
          SENDF_TO_CLIENT_OR_FAIL ("<data>"
                                   "<name>%s</name>"
                                   "%s"
          ...
          if (strcmp (name, "scp_credential") == 0
          ...
          else
            {
              SENDF_TO_CLIENT_OR_FAIL ("<data>"
                                       ...
                                       "%s"
                                       "</data>",
                                       name, alert_data_iterator_data (&data));
            }
        }
      /* If there is no email message data, send the default. */
      if (strcmp (method, "Email") == 0 && message == 0
          && (notice == 0 || notice == 2))
        SENDF_TO_CLIENT_OR_FAIL ("<data>"
                                 "<name>message</name>"
                                 "%s"
                                 "</data>",
                                 notice == 0 ? ALERT_MESSAGE_INCLUDE
      ...
      if (get_alerts_data->tasks)
        {
          ...
          init_alert_task_iterator (&tasks, get_iterator_resource (&alerts), 0);
          while (next (&tasks))
            {
  ...
  filtered = get_alerts_data->get.id ? 1 : alert_count (&get_alerts_data->get);
...
handle_get_assets (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_assets") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_assets", "Permission denied"));
  ...
  if (get_assets_data->type == NULL)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_assets", "No type specified."));
      get_assets_data_reset (get_assets_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if (ret)
    {
      switch (ret)
        ...
        case 1:
          ...
          break;
        case 2:
          if (send_find_error_to_client ("get_assets", "filter",
                                         get_assets_data->get.filt_id,
                                         gmp_parser))
            {
              error_send_to_client (error);
              return;
          ...
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_assets"));
          break;
        }
      get_assets_data_reset (get_assets_data);
      set_client_state (CLIENT_AUTHENTIC);
  ...
  count = 0;
  manage_filter_controls (get_assets_data->get.filter, &first, NULL, NULL,
                          NULL);
  ...
  while (next (&assets))
    {
      ...
      if (send_get_common (
            "asset", &get_assets_data->get, &assets, gmp_parser->client_writer,
            gmp_parser->client_writer_data, asset_iterator_writable (&assets),
      ...
      if (g_strcmp0 ("host", get_assets_data->type) == 0)
        {
          ...
          init_host_identifier_iterator (
            &identifiers, get_iterator_resource (&assets), 0, NULL);
          while (next (&identifiers))
            {
              const char *source_type;
              gchar *name;

              source_type = host_identifier_iterator_source_type (&identifiers);
              if (strcmp (source_type, "User") == 0)
                name =
                  user_name (host_identifier_iterator_source_id (&identifiers));
              else
                name = NULL;
              ...
              xml_string_append (
                ...
                host_identifier_iterator_source_id (&identifiers), source_type,
                host_identifier_iterator_source_data (&identifiers),
                host_identifier_iterator_source_orphan (&identifiers),
                name ? name : "");
              ...
              if (strcmp (get_iterator_name (&identifiers), "OS") == 0)
                xml_string_append (
                  ...
                  "<title>%s</title>"
                  "</os>",
                  host_identifier_iterator_os_id (&identifiers),
                  host_identifier_iterator_os_title (&identifiers));
      ...
      g_string_append_printf (result, "<type>%s</type>", get_assets_data->type);
      ...
      if (g_strcmp0 ("os", get_assets_data->type) == 0)
        {
          ...
          g_string_append_printf (result,
                                  ...
                                  asset_os_iterator_installs (&assets));
          init_os_host_iterator (&os_hosts, get_iterator_resource (&assets));
          while (next (&os_hosts))
            {
              const char *severity;
      ...
      else if (g_strcmp0 ("host", get_assets_data->type) == 0)
        {
          ...
          init_host_detail_iterator (&details, get_iterator_resource (&assets));
          while (next (&details))
            g_string_append_printf (
              result,
              "<detail>"
              ...
              "</source>"
              "</detail>",
              host_detail_iterator_name (&details),
              host_detail_iterator_value (&details),
              host_detail_iterator_source_id (&details),
              host_detail_iterator_source_type (&details));
  ...
  filtered = get_assets_data->get.id ? 1 : asset_count (&get_assets_data->get);
...
handle_get_configs (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_configs", "config",
                                         get_configs_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
  ...
  while (1)
    {
      ...
      ret = get_next (&configs, &get_configs_data->get, &first, &count,
      ...
      config_families_growing = config_iterator_families_growing (&configs);
      ...
      SENDF_TO_CLIENT_OR_FAIL (
        ...
        config_iterator_family_count (&configs), config_families_growing,
        config_iterator_nvt_count (&configs), config_nvts_growing, config_type,
        usage_type, config_iterator_predefined (&configs));

      if (config_type == 0
          && (get_configs_data->families || get_configs_data->get.details))
        {
          iterator_t families;
          ...
          while (next (&families))
            {
              ...
              if (family)
              ...
              else
                {
                  ...
                  family_max = -1;
                  family_selected_count =
                    nvt_selector_nvt_count (selector, NULL, 0);
                }
              ...
              SENDF_TO_CLIENT_OR_FAIL (
                ...
                "<growing>%i</growing>"
                "</family>",
                family ? family : "", family_selected_count, family_max,
                family_growing);
              if (family_max > 0)
      ...
      else if (get_configs_data->preferences || get_configs_data->get.details)
        {
          iterator_t prefs;
          ...
          while (next (&prefs))
            {
              ...
              if (timeout && strlen (timeout))
                SENDF_TO_CLIENT_OR_FAIL (
                  ...
                  config_timeout_iterator_nvt_name (&prefs), timeout);
            }
          cleanup_iterator (&prefs);
          ...
          while (next (&selectors))
            {
              ...
              SENDF_TO_CLIENT_OR_FAIL (
                ...
                nvt_selector_iterator_include (&selectors), type,
      ...
      if (get_configs_data->tasks)
        {
          ...
          SEND_TO_CLIENT_OR_FAIL ("<tasks>");
          init_config_task_iterator (&tasks, get_iterator_resource (&configs),
                                     0);
          while (next (&tasks))
            {
  ...
  filtered =
    get_configs_data->get.id ? 1 : config_count (&get_configs_data->get);
...
handle_get_credentials (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  ret = init_credential_iterator (&credentials, &get_credentials_data->get);
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_credentials", "credential",
        ...
        case 2:
          ...
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_credentials"));
          break;
        }
  ...
  while (1)
    {
      ...
      ret = get_next (&credentials, &get_credentials_data->get, &first, &count,
      ...
      SENDF_TO_CLIENT_OR_FAIL (
        ...
        credential_iterator_allow_insecure (&credentials), login ? login : "",
        type ? type : "", type ? credential_full_type (type) : "");
      ...
      if (type && (strcmp (type, "snmp") == 0))
        {
          ...
          auth_algorithm = credential_iterator_auth_algorithm (&credentials);
          privacy_algorithm =
      ...
      if (cert && get_credentials_data->get.details)
        {
          ...
          get_certificate_info (cert, -1, &activation_time, &expiration_time,
                                &md5_fingerprint, NULL, /* sha256_fingerprint */
                                NULL,                   /* subject */
                                &issuer, NULL,          /* serial */
          ...
          SENDF_TO_CLIENT_OR_FAIL (
            ...
            md5_fingerprint ? md5_fingerprint : "", issuer ? issuer : "");
      ...
      switch (format)
        ...
        case CREDENTIAL_FORMAT_KEY:
          {
            if (public_key && strcmp (public_key, ""))
              {
                SENDF_TO_CLIENT_OR_FAIL ("<public_key>%s</public_key>",
                                         public_key);
              }
            else
              {
        ...
        case CREDENTIAL_FORMAT_RPM:
          ...
          SENDF_TO_CLIENT_OR_FAIL ("<package format=\"rpm\">%s</package>",
                                   package ?: "");
          g_free (package);
          break;
        case CREDENTIAL_FORMAT_DEB:
          package = credential_iterator_deb (&credentials);
          SENDF_TO_CLIENT_OR_FAIL ("<package format=\"deb\">%s</package>",
                                   package ?: "");
          g_free (package);
          break;
        case CREDENTIAL_FORMAT_EXE:
          package = credential_iterator_exe (&credentials);
          SENDF_TO_CLIENT_OR_FAIL ("<package format=\"exe\">%s</package>",
                                   package ?: "");
          g_free (package);
          break;
        case CREDENTIAL_FORMAT_PEM:
          {
            SENDF_TO_CLIENT_OR_FAIL ("<certificate>%s</certificate>",
                                     cert ?: "");
            break;
          }
  ...
  SEND_GET_END ("credential", &get_credentials_data->get, count, filtered);
  get_credentials_data_reset (get_credentials_data);
  set_client_state (CLIENT_AUTHENTIC);
}
...
get_feed_lock_status (const char *lockfile_name, gchar **timestamp)
{
  ...
  old_umask = umask (0);
  lockfile = open (lockfile_name, O_RDWR | O_CREAT,
                   /* "-rw-rw-r--" */
...
get_nvt_feed (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (gvm_get_sync_script_description (nvt_sync_script, &feed_description)
      && gvm_get_sync_script_identification (nvt_sync_script,
                                             &feed_identification, NVT_FEED)
      && gvm_get_sync_script_feed_version (nvt_sync_script, &feed_version))
    {
      ...
      if (ident[0] == NULL || ident[1] == NULL || ident[2] == NULL
          || ident[3] == NULL)
        {
          g_strfreev (ident);
        ...
        {
          SENDF_TO_CLIENT_OR_FAIL ("<feed>"
                                   "<type>NVT</type>"
                                   "<name>%s</name>"
                                   "<version>%s</version>"
                                   "<description>%s</description>",
                                   ident[3], feed_version, feed_description);
          g_strfreev (ident);
 ...
 */
static int
get_feed_info_parse (entity_t entity, const gchar *config_path, gchar **name,
                     gchar **version, gchar **description)
{
  entity_t child;
  ...
  *name = entity_text (child);
  ...
  if (child == NULL)
    {
      g_warning ("%s: Missing description in '%s'", __func__, config_path);
      return -1;
    }
               ...
               gchar **feed_description)
{
  ...
  config_path = g_build_filename (feed_data_dir, "feed.xml", NULL);
  ...
  if (error)
    {
      g_warning ("%s: Failed to read '%s': %s", __func__, config_path,
                 error->message);
      g_error_free (error);
      g_free (config_path);
      return -1;
...
get_feed (gmp_parser_t *gmp_parser, GError **error, int feed_type)
{
  ...
  SENDF_TO_CLIENT_OR_FAIL ("<feed>"
                           ...
                           "<version>%s</version>"
                           "<description>%s</description>",
                           feed_type_name (feed_type), feed_name, feed_version,
                           feed_description);
...
handle_get_feeds (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_feeds") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_feeds", "Permission denied"));
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
...
handle_get_filters (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_filters", "filter",
                                         get_filters_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          ...
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_filters"));
          break;
        }
  ...
  filtered =
    get_filters_data->get.id ? 1 : filter_count (&get_filters_data->get);
...
handle_get_groups (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_groups", "group",
                                         get_groups_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          ...
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_groups"));
          break;
        }
  ...
  filtered = get_groups_data->get.id ? 1 : group_count (&get_groups_data->get);
...
handle_get_info (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_info") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_info", "Permission denied"));
  ...
  if (manage_scap_loaded () == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_info", "The SCAP database is required"));
      get_info_data_reset (get_info_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  if (manage_cert_loaded () == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_info", "The CERT database is required"));
      get_info_data_reset (get_info_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if (get_info_data->name && get_info_data->get.id)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_info", "Only one of name and the id attribute"
                                      " may be given."));
      get_info_data_reset (get_info_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
  ...
  if (get_info_data->type == NULL)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_info", "No type specified."));
      get_info_data_reset (get_info_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if (g_strcmp0 ("cpe", get_info_data->type) == 0)
  ...
  else if (g_strcmp0 ("cve", get_info_data->type) == 0)
    {
      ...
      get_info_data->get.subtype = g_strdup ("cve");
           ...
           && (get_info_data->name == NULL) && (get_info_data->get.id == NULL))
    {
  ...
  else if (g_strcmp0 ("nvt", get_info_data->type) == 0)
    {
      ...
      if (result)
        ...
        {
          if (send_find_error_to_client (
                "get_info", get_info_data->name ? "name" : "ID",
                get_info_data->name ? get_info_data->name
    ...
    {
      if (send_find_error_to_client ("get_info", "type", get_info_data->type,
                                     gmp_parser))
        {
          error_send_to_client (error);
        }
  ...
  if (ret)
    {
      switch (ret)
        ...
        case 2:
          if (send_find_error_to_client (
                "get_info", "filter", get_info_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_info"));
          break;
        }
      get_info_data_reset (get_info_data);
      set_client_state (CLIENT_AUTHENTIC);
  ...
  while (next (&info))
    {
      ...
      SENDF_TO_CLIENT_OR_FAIL ("<update_time>%s</update_time>", update_time);
      ...
      if (g_strcmp0 ("cpe", get_info_data->type) == 0)
        {
          ...
          if (title)
            xml_string_append (result, "<title>%s</title>",
          ...
          xml_string_append (
            ...
            "<cve_refs>%s</cve_refs>"
            "<status>%s</status>",
            cpe_info_iterator_nvd_id (&info) ? cpe_info_iterator_nvd_id (&info)
                                             : "",
            cpe_info_iterator_score (&info), cpe_info_iterator_cve_refs (&info),
            cpe_info_iterator_status (&info) ? cpe_info_iterator_status (&info)
                                             : "");
          ...
          if (get_info_data->details == 1)
            {
              ...
              while (next (&cves))
                xml_string_append (
                  ...
                  " xmlns:vuln=\"http://scap.nist.gov/schema/vulnerability/"
                  "0.4\""
                  " xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\""
                  " xmlns:patch=\"http://scap.nist.gov/schema/patch/0.1\""
                  " xmlns:scap-core=\"http://scap.nist.gov/schema/scap-core/"
                  "0.1\""
                  " xmlns:cvss=\"http://scap.nist.gov/schema/cvss-v2/0.2\""
                  " xmlns=\"http://scap.nist.gov/schema/feed/vulnerability/"
                  "2.0\""
                  " id=\"%s\">"
                  "<vuln:cvss>"
                  "<cvss:base_metrics>"
                  ...
                  "</entry>"
                  "</cve>",
                  cve_iterator_name (&cves), cve_iterator_score (&cves) / 10.0);
              ...
              if (manage_cert_loaded ())
                {
                  init_cve_cert_bund_adv_iterator (
                    &cert_advs, get_iterator_name (&info), 1, NULL);
                  ...
                  while (next (&cert_advs))
                    {
                      xml_string_append (
                        ...
                        "</cert_ref>",
                        get_iterator_name (&cert_advs),
                        dfn_cert_adv_info_iterator_title (&cert_advs));
                    };
                  cleanup_iterator (&cert_advs);
                }
              else
      ...
      else if (g_strcmp0 ("ovaldef", get_info_data->type) == 0)
        {
          ...
          if (get_info_data->details == 1)
            xml_string_append (result, "<description>%s</description>",
                               description);
        }
      else if (g_strcmp0 ("cert_bund_adv", get_info_data->type) == 0)
        xml_string_append (result,
        ...
        {
          if (send_nvt (&info, 1, 1, -1, NULL, 0, gmp_parser->client_writer,
      ...
      if (get_info_data->details == 1)
        {
          ...
          manage_read_info (get_info_data->type, nonconst_id, nonconst_name,
                            &raw_data);
...
handle_get_notes (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (get_notes_data->note_id && get_notes_data->nvt_oid)
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("get_notes", "Only one of NVT and the note_id attribute"
                                     " may be given"));
  else if (get_notes_data->note_id && get_notes_data->task_id)
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("get_notes", "Only one of the note_id and task_id"
    ...
    {
      if (send_find_error_to_client ("get_notes", "task",
                                     get_notes_data->task_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
    }
  else if (get_notes_data->nvt_oid && find_nvt (get_notes_data->nvt_oid, &nvt))
    SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_notes"));
  else if (get_notes_data->nvt_oid && nvt == 0)
    {
      if (send_find_error_to_client ("get_notes", "NVT",
                                     get_notes_data->nvt_oid, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
  ...
  else
    {
      ...
      ret = init_note_iterator (&notes, &get_notes_data->get, nvt, 0, task);
      if (ret)
        {
          switch (ret)
            {
            case 1:
              if (send_find_error_to_client (
                    "get_notes", "note", get_notes_data->get.id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
              break;
            case 2:
              if (send_find_error_to_client ("get_notes", "filter",
...
handle_get_nvts (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_nvts") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_nvts", "Permission denied"));
  ...
  if (feed_version)
    {
      ...
      if (get_nvts_data->nvt_oid && get_nvts_data->family)
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("get_nvts", "Too many parameters at once"));
      else if ((get_nvts_data->details == 0) && get_nvts_data->preference_count)
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("get_nvts", "The preference_count attribute"
                                        " requires the details attribute"));
      else if ((get_nvts_data->details == 0) && get_nvts_data->preferences)
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("get_nvts", "The preferences attribute"
                                        " requires the details attribute"));
        ...
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("get_nvts", "The timeout attribute"
                                        " requires the details and config_id"
                                        " attributes"));
      else if (get_nvts_data->nvt_oid
               && find_nvt (get_nvts_data->nvt_oid, &nvt))
        SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_nvts"));
      else if (get_nvts_data->nvt_oid && nvt == 0)
        {
          if (send_find_error_to_client ("get_nvts", "NVT",
                                         get_nvts_data->nvt_oid, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        }
      else if (get_nvts_data->config_id && get_nvts_data->preferences_config_id)
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("get_nvts", "config_id and"
        ...
        {
          if (send_find_error_to_client ("get_nvts", "config",
                                         get_nvts_data->config_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
               ...
               && find_config_with_permission (
                 get_nvts_data->preferences_config_id, &preferences_config,
                 NULL))
        SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_nvts"));
      else if (get_nvts_data->preferences_config_id
      ...
      else
        {
          ...
          SENDF_TO_CLIENT_OR_FAIL ("<get_nvts_response"
                                   " status=\"" STATUS_OK "\""
                                   " status_text=\"" STATUS_OK_TEXT "\">");
            ...
            while (next (&nvts))
              {
                ...
                if (get_nvts_data->timeout)
                  timeout =
                    config_nvt_timeout (config, nvt_iterator_oid (&nvts));
                ...
                if (send_nvt (&nvts, 1, get_nvts_data->preferences, pref_count,
                              timeout, config, gmp_parser->client_writer,
                              gmp_parser->client_writer_data))
                  {
                    free (timeout);
                    cleanup_iterator (&nvts);
...
handle_get_nvt_families (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_nvt_families") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_nvt_families", "Permission denied"));
  ...
  init_family_iterator (&families, 1, NULL, get_nvt_families_data->sort_order);
  while (next (&families))
    {
      int family_max;
      const char *family;
      ...
      SENDF_TO_CLIENT_OR_FAIL ("<family>"
                               "<name>%s</name>"
                               /* The total number of NVT's in the family. */
                               "<max_nvt_count>%i</max_nvt_count>"
                               "</family>",
                               family ? family : "", family_max);
    }
  cleanup_iterator (&families);
...
handle_get_overrides (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (get_overrides_data->override_id && get_overrides_data->nvt_oid)
    SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
      "get_overrides", "Only one of NVT and the override_id attribute"
                       " may be given"));
  else if (get_overrides_data->override_id && get_overrides_data->task_id)
    SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
      "get_overrides", "Only one of the override_id and task_id"
                       " attributes may be given"));
  ...
  else if (get_overrides_data->task_id && task == 0)
    {
      if (send_find_error_to_client ("get_overrides", "task",
                                     get_overrides_data->task_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
    ...
    {
      if (send_find_error_to_client ("get_overrides", "NVT",
                                     get_overrides_data->nvt_oid, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
  ...
  else
    {
      ...
      ret = init_override_iterator (&overrides, &get_overrides_data->get, nvt,
                                    0, task);
      if (ret)
        {
          switch (ret)
            {
            case 1:
              if (send_find_error_to_client ("get_overrides", "override",
              ...
              break;
            case 2:
              if (send_find_error_to_client ("get_overrides", "filter",
      ...
      // TODO: Do the iteration with get_next so it checks "first".
      buffer_overrides_xml (buffer, &overrides, get_overrides_data->get.details,
      ...
      filtered = get_overrides_data->get.id
                   ? 1
                   : override_count (&get_overrides_data->get, nvt, 0, task);
      SEND_GET_END ("override", &get_overrides_data->get, count, filtered);
    }
  get_overrides_data_reset (get_overrides_data);
  set_client_state (CLIENT_AUTHENTIC);
}
...
handle_get_permissions (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  ret = init_permission_iterator (&permissions, &get_permissions_data->get);
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_permissions", "permission",
          ...
          break;
        case 2:
          if (send_find_error_to_client ("get_permissions", "filter",
          ...
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_permissions"));
          break;
        }
  ...
  while (1)
    {
      ...
      ret = get_next (&permissions, &get_permissions_data->get, &first, &count,
...
handle_get_port_lists (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  ret = init_port_list_iterator (&port_lists, &get_port_lists_data->get);
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_port_lists", "port_list",
  ...
  while (1)
    {
      ret = get_next (&port_lists, &get_port_lists_data->get, &first, &count,
      ...
      SEND_GET_COMMON (port_list, &get_port_lists_data->get, &port_lists);
      ...
      if (get_port_lists_data->get.details)
        {
          ...
          init_port_range_iterator (
            &ranges, get_iterator_resource (&port_lists), 0, 1, NULL);
          while (next (&ranges))
            SENDF_TO_CLIENT_OR_FAIL ("<port_range id=\"%s\">"
                                     "<start>%s</start>"
                                     "<end>%s</end>"
                                     "<type>%s</type>"
                                     "<comment>%s</comment>"
      ...
      if (get_port_lists_data->targets)
        {
          ...
          init_port_list_target_iterator (
            &targets, get_iterator_resource (&port_lists), 0);
          while (next (&targets))
            {
...
handle_get_preferences (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_preferences") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_preferences", "Permission denied"));
  ...
  if (get_preferences_data->nvt_oid
  ...
  else if (get_preferences_data->nvt_oid && nvt == 0)
    {
      if (send_find_error_to_client ("get_preferences", "NVT",
                                     get_preferences_data->nvt_oid, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
        ...
        while (next (&prefs))
          {
            ...
            if (name)
              name = strstr (name + 1, ":");
            if (name
                && (strcmp (name + 1, get_preferences_data->preference) == 0))
              {
                GString *buffer = g_string_new ("");
                buffer_config_preference_xml (buffer, &prefs, config, 1);
                SEND_TO_CLIENT_OR_FAIL (buffer->str);
...
handle_get_reports (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_reports") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_reports", "Permission denied"));
      get_reports_data_reset (get_reports_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if (get_reports_data->get.trash)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_reports", "Getting reports from the trashcan"
                                         " is not supported"));
      get_reports_data_reset (get_reports_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
  ...
  if (find_report_format_with_permission (get_reports_data->format_id,
                                          &report_format, "get_report_formats"))
    {
      get_reports_data_reset (get_reports_data);
      SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_reports"));
      set_client_state (CLIENT_AUTHENTIC);
  ...
  if (report_format == 0)
    {
      if (send_find_error_to_client ("get_reports", "report format",
                                     get_reports_data->format_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
  ...
  if (get_reports_data->get.filt_id
    ...
    {
      filter_t filter;
      if (find_filter_with_permission (get_reports_data->get.filt_id, &filter,
                                       "get_filters"))
        {
          get_reports_data_reset (get_reports_data);
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_reports"));
  ...
  if (get_reports_data->report_id && request_report == 0)
    {
      if (send_find_error_to_client ("get_reports", "report",
                                     get_reports_data->report_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
  ...
  if (get_reports_data->delta_report_id
      && strcmp (get_reports_data->delta_report_id, "0") && delta_report == 0)
    {
      if (send_find_error_to_client ("get_reports", "report",
                                     get_reports_data->delta_report_id,
                                     gmp_parser))
  ...
  if (report_format_active (report_format) == 0)
    {
      get_reports_data_reset (get_reports_data);
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_reports", "Report format must be active"));
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if ((report_format_predefined (report_format) == 0)
      && (report_format_trust (report_format) > 1))
    {
      get_reports_data_reset (get_reports_data);
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_reports", "Report format must be trusted"));
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }
  ...
  if (get_reports_data->get.id)
  ...
  else
    {
       ...
       * init_get. */
      ret = init_get ("get_reports", &get_reports_data->report_get, "Reports",
                      &first);
      if (ret)
        {
          switch (ret)
  ...
  if ((get_reports_data->report_get.id == NULL)
      || (strlen (get_reports_data->report_get.id) == 0))
    {
      ...
      get_reports_data->get.filter =
        g_strdup_printf ("apply_overrides=%i min_qod=%i", overrides, min_qod);
    }
  ...
  if (ret)
    {
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_reports", "report",
                                         get_reports_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
  ...
  while (next_report (&reports, &report))
    {
      ...
      if (get_reports_data->alert_id == NULL)
        g_string_append_printf (prefix,
                                ...
                                get_reports_data->format_id, extension,
                                content_type);
      ...
      if (get_reports_data->alert_id == NULL)
        {
          ...
          buffer_xml_append_printf (
            ...
            get_iterator_owner_name (&reports)
              ? get_iterator_owner_name (&reports)
              : "",
            get_iterator_name (&reports) ? get_iterator_name (&reports) : "",
            get_iterator_comment (&reports) ? get_iterator_comment (&reports)
                                            : "",
          ...
          if (task)
            {
              ...
              buffer_xml_append_printf (prefix, "<task id=\"%s\">",
                                        report_task_uuid);
              ...
              if (strcmp (get_reports_data->format_id,
                          "5057e5cc-b825-11e4-9d0e-28d24461215b"))
                {
                  ...
                  buffer_xml_append_printf (prefix, "<name>%s</name>",
                                            report_task_name);
      ...
      ret = manage_send_report (
        report, delta_report, report_format, &get_reports_data->get,
        get_reports_data->notes_details, get_reports_data->overrides_details,
        get_reports_data->result_tags, get_reports_data->ignore_pagination,
        ...
        get_reports_data->format_id
          ...
          && strcmp (get_reports_data->format_id,
                     "5057e5cc-b825-11e4-9d0e-28d24461215b"),
        send_to_client, gmp_parser->client_writer,
        gmp_parser->client_writer_data, get_reports_data->alert_id,
        prefix->str);
          ...
          if (get_reports_data->alert_id)
            switch (ret)
              ...
              case 0:
                break;
              case 1:
                if (send_find_error_to_client ("get_reports", "alert",
                                               get_reports_data->alert_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case -2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "get_reports", "Failed to find report format for"
                                 " alert"));
                if (request_report == 0)
                  cleanup_iterator (&reports);
                get_reports_data_reset (get_reports_data);
              ...
              case -4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("get_reports", "Failed to find filter for"
                                                   " alert"));
                if (request_report == 0)
                  cleanup_iterator (&reports);
                get_reports_data_reset (get_reports_data);
  ...
  if (get_reports_data->alert_id)
  ...
  else
    {
      ...
      SEND_GET_END ("report", &get_reports_data->report_get, count, filtered);
    }
...
{
  if (get_report_formats_data->params && get_report_formats_data->get.trash)
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("get_report_formats", "Params given with trash"));
  else
    {
      ...
      if (ret)
        {
          switch (ret)
            ...
            case 1:
              if (send_find_error_to_client (
                    "get_report_formats", "report_format",
                    get_report_formats_data->get.id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
      ...
      while (1)
        {
          ...
          ret = get_next (&report_formats, &get_report_formats_data->get,
                          &first, &count, init_report_format_iterator);
          if (ret == 1)
            break;
          if (ret == -1)
            {
          ...
          SEND_GET_COMMON (report_format, &get_report_formats_data->get,
                           &report_formats);

          trust_time = report_format_iterator_trust_time (&report_formats);
          ...
          if (get_report_formats_data->alerts)
            {
              ...
              init_report_format_alert_iterator (
                &alerts, get_iterator_resource (&report_formats));
              while (next (&alerts))
                {
              ...
              while (next (&params))
                {
                  ...
                  if (report_format_param_iterator_type (&params)
                      == REPORT_FORMAT_PARAM_TYPE_REPORT_FORMAT_LIST)
                    {
                      ...
                      SENDF_TO_CLIENT_OR_FAIL ("</type><value>%s",
                                               value ? value : "");
                      if (value)
                        {
                          ...
                          while (*current_id)
                            {
                              ...
                              find_report_format_with_permission (
                                *current_id, &value_rf, "get_report_formats");
                              name =
                                value_rf ? report_format_name (value_rf) : NULL;
                      ...
                      SENDF_TO_CLIENT_OR_FAIL ("</value><default>%s",
                                               fallback ? fallback : "");
                      if (fallback)
                        {
                          gchar **ids, **current_id;
                      ...
                      SENDF_TO_CLIENT_OR_FAIL ("</default>");
                    }
                  else
                    {
                      SENDF_TO_CLIENT_OR_FAIL (
                  ...
                  if (report_format_param_iterator_type (&params)
                      == REPORT_FORMAT_PARAM_TYPE_SELECTION)
                    {
                      ...
                      init_param_option_iterator (
                        &options, report_format_param_iterator_param (&params),
                        1, NULL);
                      while (next (&options))
                        SENDF_TO_CLIENT_OR_FAIL (
                          "<option>%s</option>",
              ...
              while (next_file (&files))
                {
                  ...
                  SENDF_TO_CLIENT_OR_FAIL ("<file name=\"%s\">%s</file>",
      ...
      SEND_GET_END ("report_format", &get_report_formats_data->get, count,
                    filtered);
    }
  get_report_formats_data_reset (get_report_formats_data);
  set_client_state (CLIENT_AUTHENTIC);
...
handle_get_results (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_results") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_results", "Permission denied"));
  ...
  if (get_results_data->get.trash)
    {
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "get_results", "Getting results from the trashcan is not"
                       " supported"));
      get_results_data_reset (get_results_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
  ...
  if (get_results_data->get.id
      && find_result_with_permission (get_results_data->get.id, &result, NULL))
    SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_results"));
  else if (get_results_data->get.id && result == 0)
    {
      if (send_find_error_to_client ("get_results", "result",
                                     get_results_data->get.id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
           ...
           && find_task_with_permission (get_results_data->task_id, &task,
                                         NULL))
    SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_results"));
  else if (get_results_data->task_id && task == 0)
    {
      if (send_find_error_to_client ("get_results", "task",
                                     get_results_data->task_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
  ...
  else
    {
      ...
      report_id = filter_term_value (filter, "_and_report_id");
      report = 0;

      if (report_id)
        {
          if (find_report_with_permission (report_id, &report, NULL))
            {
              g_free (report_id);
      ...
      manage_report_filter_controls (filter, NULL, /* first */
                                     NULL,         /* max */
                                     NULL,         /* sort_field */
                                     NULL,         /* sort_order */
          ...
          do
            {
              GString *buffer = g_string_new ("");
              buffer_results_xml (
                buffer, &results, task, notes, get_results_data->notes_details,
                overrides, get_results_data->overrides_details, 1,
                /* show tag details if selected by ID */
                get_results_data->get.id != NULL, get_results_data->get.details,
                NULL, NULL, 0, -1, 0); /* Lean. */
              SEND_TO_CLIENT_OR_FAIL (buffer->str);
              g_string_free (buffer, TRUE);
              count++;
      ...
      manage_filter_controls (get_results_data->get.filter, &first, NULL, NULL,
                              NULL);
      ...
      if (get_results_data->get_counts)
        {
          ...
          filtered =
            get_results_data->get.id
              ? 1
              : result_count (&get_results_data->get, report /* No report */,
                              NULL /* No host */);
      ...
      else if (send_get_end_no_counts ("result", &get_results_data->get,
                                       gmp_parser->client_writer,
                                       gmp_parser->client_writer_data))
        {
          error_send_to_client (error);
...
handle_get_roles (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      switch (ret)
        ...
        case 1:
          if (send_find_error_to_client ("get_roles", "role",
                                         get_roles_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          if (send_find_error_to_client (
                "get_roles", "role", get_roles_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_roles"));
          break;
        }
  ...
  filtered = get_roles_data->get.id ? 1 : role_count (&get_roles_data->get);
...
handle_get_scanners (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  switch (ret)
    ...
    case 0:
      break;
    case 1:
      if (send_find_error_to_client ("get_scanners", "scanners",
                                     get_scanners_data->get.id, gmp_parser))
        {
          error_send_to_client (error);
          break;
        }
      break;
    case 2:
      if (send_find_error_to_client ("get_scanners", "filter",
                                     get_scanners_data->get.filt_id,
                                     gmp_parser))
        {
          error_send_to_client (error);
          break;
  ...
  while (1)
    {
          ...
          if (scanner_iterator_ca_pub (&scanners))
            {
              ...
              SENDF_TO_CLIENT_OR_FAIL (
                ...
                "</ca_pub_info>",
                certificate_time_status (activation_time, expiration_time),
                activation_time_str, expiration_time_str, md5_fingerprint,
                issuer);
              g_free (activation_time_str);
              g_free (expiration_time_str);
              g_free (md5_fingerprint);
      ...
      if ((scanner_iterator_type (&scanners) == SCANNER_TYPE_OSP
          ...
          && get_scanners_data->get.details)
        {
          ...
          if (!osp_get_version_from_iterator (&scanners, &s_name, &s_ver,
                                              &d_name, &d_ver, &p_name, &p_ver)
  ...
  cleanup_iterator (&scanners);
  filtered =
    get_scanners_data->get.id ? 1 : scanner_count (&get_scanners_data->get);
...
handle_get_schedules (gmp_parser_t *gmp_parser, GError **error)
{
  if (get_schedules_data->tasks && get_schedules_data->get.trash)
    SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
      "get_schedules", "Attributes tasks and trash both given"));
  else
    {
      ...
      if (ret)
        {
          switch (ret)
            {
            case 1:
              if (send_find_error_to_client ("get_schedules", "schedule",
              ...
              break;
            case 2:
              if (send_find_error_to_client ("get_schedules", "filter",
      ...
      while (1)
        {
          ...
          ret = get_next (&schedules, &get_schedules_data->get, &first, &count,
          ...
          SENDF_TO_CLIENT_OR_FAIL ("<icalendar>%s</icalendar>"
                                   "<timezone>%s</timezone>",
                                   icalendar ? icalendar : "",
          ...
          if (get_schedules_data->tasks)
            {
              ...
              init_schedule_task_iterator (&tasks,
                                           get_iterator_resource (&schedules));
              while (next (&tasks))
                {
                  SENDF_TO_CLIENT_OR_FAIL (
                    "<task id=\"%s\">"
  ...
  if (create_schedule_data->copy)
    {
      switch (copy_schedule (create_schedule_data->name,
                             ...
                             create_schedule_data->copy, &new_schedule))
        {
        case 0:
          {
            char *uuid;
        ...
        case 1:
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("create_schedule", "Schedule exists already"));
        ...
        case 99:
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("create_schedule", "Permission denied"));
          log_event_fail ("schedule", "Schedule", NULL, "created");
          break;
        case -1:
        default:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_schedule"));
          log_event_fail ("schedule", "Schedule", NULL, "created");
          break;
        }
    ...
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("create_schedule", "A NAME entity is required"));
      goto create_schedule_leave;
    }
    ...
    {
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "create_schedule", "An ICALENDAR entity is required"));
      goto create_schedule_leave;
    }

  switch (create_schedule (
    create_schedule_data->name, create_schedule_data->comment,
    create_schedule_data->icalendar, create_schedule_data->timezone,
    &new_schedule, &ical_error))
    {
    case 0:
      {
        char *uuid = schedule_uuid (new_schedule);
        SENDF_TO_CLIENT_OR_FAIL ("<create_schedule_response status=\"201\""
                                 " status_text=\"OK, resource created\""
                                 " id=\"%s\">",
                                 uuid);
        if (ical_error)
          {
            SEND_TO_CLIENT_OR_FAIL ("<status_details>");
            SEND_TO_CLIENT_OR_FAIL (ical_error ? ical_error : "");
            SEND_TO_CLIENT_OR_FAIL ("</status_details>");
          }
        SEND_TO_CLIENT_OR_FAIL ("</create_schedule_response>");
        log_event ("schedule", "Schedule", uuid, "created");
        free (uuid);
        break;
      }
    ...
    case 3:
      {
        SENDF_TO_CLIENT_OR_FAIL ("<create_schedule_response status=\"400\""
    ...
    case 4:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("create_schedule", "Error in TIMEZONE"));
      log_event_fail ("schedule", "Schedule", NULL, "created");
      break;
    case 99:
      SEND_TO_CLIENT_OR_FAIL (
...
handle_modify_schedule (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (modify_schedule_data->icalendar == NULL
    ...
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_schedule", "ICALENDAR element is required"));
  ...
  switch (modify_schedule (
    modify_schedule_data->schedule_id, modify_schedule_data->name,
    modify_schedule_data->comment, modify_schedule_data->icalendar,
    modify_schedule_data->timezone, &ical_error))
    {
    case 0:
      SENDF_TO_CLIENT_OR_FAIL ("<modify_schedule_response status=\"200\""
                               " status_text=\"OK\">"
                               "<status_details>%s</status_details>"
                               "</modify_schedule_response>",
                               ical_error ? ical_error : "");
      log_event ("schedule", "Schedule", modify_schedule_data->schedule_id,
                 "modified");
      break;
    case 1:
      ...
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    case 2:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "modify_schedule", "Schedule with new name exists already"));
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    case 3:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_schedule", "Error in type name"));
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    case 4:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "modify_schedule", "MODIFY_SCHEDULE requires a schedule_id"));
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    case 6:
      {
        SENDF_TO_CLIENT_OR_FAIL ("<modify_schedule_response status=\"400\""
                                 " status_text=\"Invalid ICALENDAR: %s\">"
                                 "</modify_schedule_response>",
                                 ical_error);
    ...
    case 7:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_schedule", "Error in TIMEZONE"));
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    case 99:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_schedule", "Permission denied"));
      log_event_fail ("schedule", "Schedule", modify_schedule_data->schedule_id,
                      "modified");
      break;
    default:
...
handle_get_settings (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (acl_user_may ("get_settings") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_settings", "Permission denied"));
      get_settings_data_reset (get_settings_data);
      set_client_state (CLIENT_AUTHENTIC);
      return;
    }

  init_setting_iterator (&settings, get_settings_data->setting_id,
                         get_settings_data->filter, get_settings_data->first,
                         get_settings_data->max, get_settings_data->sort_order,
                         get_settings_data->sort_field);
  ...
  SENDF_TO_CLIENT_OR_FAIL (
    ...
    get_settings_data->filter ? get_settings_data->filter : "",
    /* Add 1 for 1 indexing. */
    get_settings_data->first + 1, get_settings_data->max);
  count = 0;
  while (next (&settings))
    {
      ...
      if (setting_is_default_ca_cert (setting_iterator_uuid (&settings))
          ...
          && strlen (setting_iterator_value (&settings)))
        {
          ...
          SENDF_TO_CLIENT_OR_FAIL (
            ...
            "</certificate_info>",
            certificate_time_status (activation_time, expiration_time),
            activation_time_str, expiration_time_str, md5_fingerprint, issuer);
          g_free (activation_time_str);
          g_free (expiration_time_str);
          g_free (md5_fingerprint);
          g_free (issuer);
  ...
  filtered = setting ? 1 : setting_count (get_settings_data->filter);
...
handle_get_system_reports (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  ret = init_system_report_type_iterator (&types, get_system_reports_data->name,
                                          get_system_reports_data->slave_id);
  switch (ret)
    {
    case 1:
      if (send_find_error_to_client ("get_system_reports", "system report",
                                     get_system_reports_data->name, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
      break;
    case 2:
      if (send_find_error_to_client ("get_system_reports", "slave",
                                     get_system_reports_data->slave_id,
                                     gmp_parser))
        {
          error_send_to_client (error);
    ...
    case 4:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_UNAVAILABLE (
        "get_system_reports", "Could not connect to slave"));
      break;
    case 5:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_UNAVAILABLE (
        "get_system_reports", "Authentication to slave failed"));
      break;
    case 6:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_UNAVAILABLE (
        "get_system_reports", "Failed to get system report from slave"));
      break;
    case 99:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_system_reports", "Permission denied"));
      break;
    default:
      assert (0);
      /* fallthrough */
    case -1:
      SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_system_reports"));
      break;
    case 0:
    case 3:
      {
        ...
        while (next_report_type (&types))
          if (get_system_reports_data->brief && (ret != 3))
            SENDF_TO_CLIENT_OR_FAIL ("<system_report>"
                                     "<name>%s</name>"
                                     "<title>%s</title>"
                                     "</system_report>",
                      ...
                      get_system_reports_data->start_time,
                      get_system_reports_data->end_time,
                      get_system_reports_data->slave_id, &report))
                   && (report_ret != 3))
            {
...
handle_get_tags (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_tags", "tag",
                                         get_tags_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          if (send_find_error_to_client (
                "get_tags", "filter", get_tags_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
          break;
        default:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_tags"));
        }
  ...
  while (1)
    {
      ret = get_next (&tags, &get_tags_data->get, &first, &count,
                      get_tags_data->names_only ? init_tag_name_iterator
                                                : init_tag_iterator);
      if (ret == 1)
        break;
      if (ret == -1)
      ...
      if (get_tags_data->names_only)
      ...
      else
        {
          ...
          SENDF_TO_CLIENT_OR_FAIL ("<resources>"
                                   ...
                                   tag_iterator_resources (&tags), value,
  ...
  cleanup_iterator (&tags);
  filtered = get_tags_data->get.id ? 1 : tag_count (&get_tags_data->get);
...
handle_get_targets (gmp_parser_t *gmp_parser, GError **error)
{
  if (get_targets_data->tasks && get_targets_data->get.trash)
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("get_target", "GET_TARGETS tasks given with trash"));
  else
    {
      iterator_t targets;
      int count, filtered, ret, first;
      ...
      if (ret)
        {
          switch (ret)
            {
            case 1:
              if (send_find_error_to_client ("get_targets", "target",
              ...
              break;
            case 2:
              if (send_find_error_to_client ("get_targets", "filter",
      ...
      while (1)
        {
          ...
          ret = get_next (&targets, &get_targets_data->get, &first, &count,
          ...
          if (get_targets_data->get.trash
          ...
          else if (ssh_credential)
            {
              ...
              if (find_credential_with_permission (ssh_uuid, &found,
                                                   "get_credentials"))
                abort ();
          ...
          if (get_targets_data->get.trash
          ...
          else if (smb_credential)
            {
              ...
              if (find_credential_with_permission (smb_uuid, &found,
                                                   "get_credentials"))
                abort ();
          ...
          if (get_targets_data->get.trash
              && target_iterator_esxi_trash (&targets))
            {
              ...
              esxi_uuid = trash_credential_uuid (esxi_credential);
          ...
          else if (esxi_credential)
            {
              ...
              if (find_credential_with_permission (esxi_uuid, &found,
                                                   "get_credentials"))
                abort ();
          ...
          if (get_targets_data->get.trash
            ...
            {
              snmp_name = trash_credential_name (snmp_credential);
              snmp_uuid = trash_credential_uuid (snmp_credential);
          ...
          else if (snmp_credential)
            {
              ...
              if (find_credential_with_permission (snmp_uuid, &found,
                                                   "get_credentials"))
                abort ();
          ...
          if (port_list_trash)
            port_list_available =
              trash_port_list_readable_uuid (port_list_uuid);
          else if (port_list_uuid)
            {
              port_list_t found;
              if (find_port_list_with_permission (port_list_uuid, &found,
                                                  "get_port_lists"))
                abort ();
          ...
          SENDF_TO_CLIENT_OR_FAIL (
            ...
            "<name>%s</name>"
            "<trash>%i</trash>",
            hosts, exclude_hosts ? exclude_hosts : "", max_hosts,
            port_list_uuid ? port_list_uuid : "",
            port_list_name ? port_list_name : "", port_list_trash);
          ...
          if (get_targets_data->get.details)
            SENDF_TO_CLIENT_OR_FAIL (
              ...
              target_port_range (get_iterator_resource (&targets)));
      ...
      cleanup_iterator (&targets);
      filtered =
        get_targets_data->get.id ? 1 : target_count (&get_targets_data->get);
  ...
  if (schedule)
    {
      ...
      if (schedule_in_trash)
      ...
      else
        {
          ...
          if (find_schedule_with_permission (task_schedule_uuid, &found,
                                             "get_schedules"))
  ...
  if (schedule_available && schedule)
    {
      ...
      if (schedule_info (schedule, schedule_in_trash, &icalendar, &zone) == 0)
        xml_string_append (xml,
                           ...
                           "<timezone>%s</timezone>"
                           "</schedule>",
                           task_schedule_uuid, task_schedule_name,
                           schedule_in_trash, icalendar ? icalendar : "",
                           zone ? zone : "");
...
handle_get_tasks (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (get_tasks_data->get.details && get_tasks_data->get.trash)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("get_task", "GET_TASKS details given with trash"));
  ...
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_tasks", "task",
                                         get_tasks_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          if (send_find_error_to_client (
                "get_tasks", "task", get_tasks_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
      ...
      if (get_tasks_data->schedules_only)
      ...
      else
        {
          ...
          if ((target == 0)
            ...
            {
              progress_xml =
                g_strdup_printf ("%i", task_upload_progress (index));
              running_report = 0;
            }
          else
            {
              ...
              progress = report_progress (running_report);
          ...
          if (first_report_id && (get_tasks_data->get.trash == 0))
            {
              ...
              if (report_counts (first_report_id, &holes_2, &infos_2, &logs,
                                 &warnings_2, &false_positives, &severity_2,
          ...
          if (second_last_report_id && (get_tasks_data->get.trash == 0))
            {
              ...
              if (((first_report_id == NULL)
                  ...
                  && report_counts (second_last_report_id, &holes_2, &infos_2,
                                    &logs, &warnings_2, &false_positives,
          ...
          if (get_tasks_data->get.trash && last_report_id)
          ...
          else if (last_report_id)
            {
              ...
              if ((first_report_id == NULL) || (second_last_report_id == NULL)
                  || (strcmp (last_report_id, first_report_id)
                      && strcmp (last_report_id, second_last_report_id)))
                {
                  if (report_counts (last_report_id, &holes, &infos, &logs,
              ...
              if (strcmp (task_iterator_usage_type (&tasks), "audit") == 0)
                {
                  ...
                  report_compliance_by_uuid (last_report_id, &compliance_yes,
                                             &compliance_no,
                  ...
                  last_report = g_strdup_printf (
                    ...
                    "</report>"
                    "</last_report>",
                    last_report_id, timestamp, scan_start, scan_end,
                ...
                last_report = g_strdup_printf (
                  ...
                  "</report>"
                  "</last_report>",
                  last_report_id, timestamp, scan_start, scan_end, holes, infos,
                  logs, warnings, false_positives, severity);
              free (scan_start);
              free (scan_end);
              g_free (timestamp);
            }
          ...
          if (target_in_trash)
          ...
          else if (target)
            {
              ...
              if (find_target_with_permission (task_target_uuid, &found,
                                               "get_targets"))
          ...
          if (task_config_in_trash (index))
            ...
            {
              config_t found;
              if (find_config_with_permission (config_uuid, &found,
                                               "get_configs"))
          ...
          if (scanner)
            {
              ...
              if (scanner_in_trash)
              ...
              else
                {
                  ...
                  if (find_scanner_with_permission (task_scanner_uuid, &found,
                                                    "get_scanners"))
                    g_error ("%s: GET_TASKS: error finding"
          ...
          config_name_escaped =
            config_name ? g_markup_escape_text (config_name, -1) : NULL;
          task_target_name_escaped =
            task_target_name ? g_markup_escape_text (task_target_name, -1)
                             : NULL;
          task_scanner_name_escaped =
            task_scanner_name ? g_markup_escape_text (task_scanner_name, -1)
                              : NULL;
          ...
          response = g_strdup_printf (
            ...
            "%s" // Schedule XML
            "%s%s",
            get_tasks_data->get.trash ? 0 : task_alterable (index),
            task_iterator_usage_type (&tasks), config_uuid ?: "",
            config_name_escaped ?: "", config_type (task_config (index)),
            task_config_in_trash (index),
            config_available ? "" : "<permissions/>", task_target_uuid ?: "",
            task_target_name_escaped ?: "", target_in_trash,
            target_available ? "" : "<permissions/>",
            task_iterator_hosts_ordering (&tasks)
              ? task_iterator_hosts_ordering (&tasks)
              : "",
            task_scanner_uuid, task_scanner_name_escaped, task_scanner_type,
            scanner_in_trash, scanner_available ? "" : "<permissions/>",
            task_iterator_run_status_name (&tasks), progress_xml,
            task_iterator_total_reports (&tasks),
            task_iterator_finished_reports (&tasks),
            get_tasks_data->get.trash
              ? ""
              : task_iterator_trend_counts (&tasks, holes, warnings, infos,
                                            severity, holes_2, warnings_2,
                                            infos_2, severity_2),
            task_schedule_xml, current_report, last_report);
          g_free (config_name);
          g_free (config_uuid);
          ...
          if (send_to_client (response, gmp_parser->client_writer,
                              gmp_parser->client_writer_data))
            {
              g_free (response);
              cleanup_iterator (&tasks);
          ...
          SENDF_TO_CLIENT_OR_FAIL (
            "<observers>%s",
            ((owner == NULL) || (strcmp (owner, current_credentials.username)))
              ? ""
              : observers);
          free (owner);
          free (observers);
          ...
          while (next (&alerts))
            {
              ...
              if (find_alert_with_permission (
                    task_alert_iterator_uuid (&alerts), &found, "get_alerts"))
                abort ();
          ...
          if (get_tasks_data->get.details || get_tasks_data->get.id)
            {
          ...
          assets_apply_overrides =
            task_preference_value (index, "assets_apply_overrides");
          ...
          SENDF_TO_CLIENT_OR_FAIL (
            ...
            "</preferences>"
            "</task>",
            max_checks ? max_checks : "4", max_hosts ? max_hosts : "20",
            source_iface ? source_iface : "", in_assets ? in_assets : "yes",
            assets_apply_overrides ? assets_apply_overrides : "yes",
            assets_min_qod ? assets_min_qod : G_STRINGIFY (MIN_QOD_DEFAULT),
  ...
  cleanup_iterator (&tasks);
  filtered = get_tasks_data->get.id ? 1 : task_count (&get_tasks_data->get);
...
handle_get_users (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_users", "user",
                                         get_users_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          if (send_find_error_to_client (
                "get_users", "user", get_users_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_users"));
          break;
        }
  ...
  while (1)
    {
      ...
      SENDF_TO_CLIENT_OR_FAIL (
        ...
        hosts_allow, hosts ? hosts : "",
        user_iterator_method (&users) ? user_iterator_method (&users) : "file");

      /* Interfaces Access */
      ifaces = user_iterator_ifaces (&users);
      ifaces_allow = user_iterator_ifaces_allow (&users);
      SENDF_TO_CLIENT_OR_FAIL ("<ifaces allow=\"%i\">%s</ifaces>", ifaces_allow,
                               ifaces ? ifaces : "");

      /* User Roles */
      init_user_role_iterator (&roles, get_iterator_resource (&users));
      while (next (&roles))
        {
          SENDF_TO_CLIENT_OR_FAIL ("<role id=\"%s\">"
                                   "<name>%s</name>",
      ...
      init_user_group_iterator (&groups, get_iterator_resource (&users));
      while (next (&groups))
        {
          SENDF_TO_CLIENT_OR_FAIL ("<group id=\"%s\">"
                                   "<name>%s</name>",
  ...
  filtered = get_users_data->get.id ? 1 : user_count (&get_users_data->get);
  ...
  if (ret)
    {
      switch (ret)
        {
        case 1:
          if (send_find_error_to_client ("get_vulns", "vuln",
                                         get_vulns_data->get.id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
        ...
        case 2:
          if (send_find_error_to_client (
                "get_vulns", "filter", get_vulns_data->get.filt_id, gmp_parser))
            {
              error_send_to_client (error);
              return;
            }
          break;
        case -1:
          SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("get_vulns"));
          break;
        }
  ...
  while (next (&vulns))
    {
      ...
      SENDF_TO_CLIENT_OR_FAIL (
        ...
        "<score>%i</score>"
        "<qod>%d</qod>",
        get_iterator_uuid (&vulns), get_iterator_name (&vulns),
        vuln_iterator_type (&vulns), get_iterator_creation_time (&vulns),
        get_iterator_modification_time (&vulns),
        vuln_iterator_severity (&vulns), vuln_iterator_score (&vulns),
      ...
      SENDF_TO_CLIENT_OR_FAIL ("<newest>%s</newest>", iso_time (&newest));
...
handle_sync_config (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (!sync_config_data->config_id)
    {
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "sync_config", "SYNC_CONFIG requires a config_id attribute"));
  ...
  switch (sync_config (sync_config_data->config_id))
    ...
    case 0:
      ...
      break;
    case 1:
      if (send_find_error_to_client ("sync_config", "config",
                                     sync_config_data->config_id, gmp_parser))
        {
          error_send_to_client (error);
          return;
        }
    ...
    case 2:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("sync_config", "Config not of type OSP"));
      log_event_fail ("config", "Config", sync_config_data->config_id,
                      "synchronized");
      break;
    case 3:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("sync_config", "Config has no scanner"));
      log_event_fail ("config", "Config", sync_config_data->config_id,
                      "synchronized");
      break;
    case 4:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "sync_config", "Couldn't get parameters from scanner"));
      log_event_fail ("config", "Config", sync_config_data->config_id,
                      "synchronized");
      break;
    case 99:
...
handle_create_scanner (gmp_parser_t *gmp_parser, GError **error)
{
      ...
      case 0:
        {
          ...
          SENDF_TO_CLIENT_OR_FAIL (XML_OK_CREATED_ID ("create_scanner"), uuid);
  ...
  switch (create_scanner (
    ...
    create_scanner_data->type, &new_scanner, create_scanner_data->ca_pub,
    create_scanner_data->credential_id))
    {
    case 0:
      {
    ...
    case 4:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("create_scanner", "Credential must be of type 'up'"
                                            " (username + password)"));
      log_event_fail ("scanner", "Scanner", NULL, "created");
      break;
    case 5:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("create_scanner", "Credential must be of type 'cc'"
                                            " (client certificate)"));
      log_event_fail ("scanner", "Scanner", NULL, "created");
      break;
    case 6:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "create_scanner", "Scanner type requires a credential"));
      log_event_fail ("scanner", "Scanner", NULL, "created");
      break;
    case 99:
      SEND_TO_CLIENT_OR_FAIL (
...
handle_modify_scanner (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  /* Specifying unix file socket over GMP is not allowed. */
  if (modify_scanner_data->host && *modify_scanner_data->host == '/')
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("create_scanner", "Erroneous host value."));
      goto modify_scanner_leave;
  ...
  switch (modify_scanner (
    ...
    case 2:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "modify_scanner", "Scanner with new name exists already"));
      log_event_fail ("scanner", "Scanner", modify_scanner_data->scanner_id,
                      "modified");
      break;
    case 3:
    ...
    case 6:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_scanner", "Credential must be of type 'cc'"
                                            " (client certificate)"));
      log_event_fail ("scanner", "Scanner", modify_scanner_data->scanner_id,
                      "modified");
      break;
    case 7:
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_scanner", "Credential must be of type 'up'"
                                            " (username + password)"));
      log_event_fail ("scanner", "Scanner", modify_scanner_data->scanner_id,
                      "modified");
      break;
    case 8:
      SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
        "modify_scanner", "Scanner type requires a credential"));
      log_event_fail ("scanner", "Scanner", modify_scanner_data->scanner_id,
                      "modified");
      break;
    case 99:
...
handle_modify_config (gmp_parser_t *gmp_parser, GError **error)
{
  if (acl_user_may ("modify_config") == 0)
    {
      SEND_TO_CLIENT_OR_FAIL (
        XML_ERROR_SYNTAX ("modify_config", "Permission denied"));
      set_client_state (CLIENT_AUTHENTIC);
  ...
  if (modify_config_data->config_id == NULL
    ...
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("modify_config", "A config_id attribute is required"));
      ...
      XML_ERROR_SYNTAX ("modify_config", "Permission denied"));
           ...
           || ((modify_config_data->nvt_selection_family
               ...
               && (modify_config_data->preference_name
                   ...
                   || modify_config_data->preference_nvt_oid)))
    SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
      "modify_config", "Either a PREFERENCE or an NVT_SELECTION"
                       " or a FAMILY_SELECTION is required"));
  else if (modify_config_data->nvt_selection_family)
    {
      switch (manage_set_config_nvts (modify_config_data->config_id,
                                      modify_config_data->nvt_selection_family,
                                      modify_config_data->nvt_selection))
        {
        case 0:
          SEND_TO_CLIENT_OR_FAIL (XML_OK ("modify_config"));
          log_event ("config", "Scan config", modify_config_data->config_id,
                     "modified");
          goto modify_config_leave;
        case 1:
          SEND_TO_CLIENT_OR_FAIL (
        ...
        case 3:
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "modify_config", "Attempt to modify NVT in whole-only family"));
          log_event_fail ("config", "Scan Config",
                          modify_config_data->config_id, "modified");
          goto modify_config_leave;
        case -1:
  ...
  else if (modify_config_data->families_static_all)
    {
      ...
      switch (manage_set_config_families (
        modify_config_data->config_id, modify_config_data->families_growing_all,
        modify_config_data->families_static_all,
        ...
        case 3:
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "modify_config", "Whole-only families must include entire"
    ...
    switch (manage_set_config (
      ...
      case 1:
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("modify_config", "Name must be unique"));
        goto modify_config_leave;
      case 2:
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("modify_config", "Scanner not found"));
        goto modify_config_leave;
      case 3:
        SEND_TO_CLIENT_OR_FAIL (
          XML_ERROR_SYNTAX ("modify_config", "Config is in use"));
        log_event_fail ("config", "Scan Config", modify_config_data->config_id,
                        "modified");
        goto modify_config_leave;
      case 4:
        if (send_find_error_to_client ("modify_config", "config",
    ...
    SEND_TO_CLIENT_OR_FAIL (
      XML_ERROR_SYNTAX ("modify_config", "PREFERENCE requires a NAME element"));
  else
    switch (manage_set_config_preference (
      modify_config_data->config_id, modify_config_data->preference_nvt_oid,
      modify_config_data->preference_name,
      modify_config_data->preference_value))
      {
      case 0:
  ...
  if (result->host)
    {
      for (unsigned int i = 0; i < create_report_data->result_detection->len;
           i++)
        {
          ...
          detail->ip = g_strdup (result->host);
          detail->name =
            g_strconcat ("detected_by@", detection->location, NULL);
          detail->source_desc = g_strdup ("create_report_import");
  ...
  if (gmp_parser->read_over > 1)
    ...
    switch (client_state)
      ...
      case CLIENT_AUTHENTICATE:
        switch (authenticate (&current_credentials))
          ...
          case 0: /* Authentication succeeded. */
            {
              ...
              pw_warning = gvm_validate_password (current_credentials.password,
                                                  current_credentials.username);
          ...
          case 99: /* Authentication failed. */
            ...
            free_credentials (&current_credentials);
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("authenticate", "Permission denied"));
            set_client_state (CLIENT_TOP);
            break;
      ...
      case CLIENT_DELETE_ASSET:
        if (delete_asset_data->asset_id || delete_asset_data->report_id)
          switch (delete_asset (delete_asset_data->asset_id,
            ...
            case 0:
              ...
              log_event ("asset", "Asset", delete_asset_data->asset_id,
                         "deleted");
              break;
            case 1:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_asset", "Asset is in use"));
              log_event_fail ("asset", "Asset", delete_asset_data->asset_id,
                              "deleted");
              break;
            case 2:
              if (send_find_error_to_client ("delete_asset", "asset",
                                             delete_asset_data->asset_id,
                                             gmp_parser))
                {
                  error_send_to_client (error);
            ...
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "delete_asset", "Attempt to delete a predefined asset"));
              log_event_fail ("asset", "Asset", delete_asset_data->asset_id,
                              "deleted");
              break;
            case 4:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_asset", "An asset_id or a"
            ...
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_asset", "Permission denied"));
              log_event_fail ("asset", "Asset", delete_asset_data->asset_id,
                              "deleted");
              break;
            default:
              SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("delete_asset"));
              log_event_fail ("asset", "Asset", delete_asset_data->asset_id,
                              "deleted");
            }
        else
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "delete_asset", "An asset_id attribute is required"));
        ...
        if (delete_task_data->task_id)
          {
            switch (request_delete_task_uuid (delete_task_data->task_id,
              ...
              case 2: /* Hidden task. */
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "delete_task", "Attempt to delete a hidden task"));
                log_event_fail ("task", "Task", delete_task_data->task_id,
                                "deleted");
                break;
              case 3: /* Failed to find task. */
                if (send_find_error_to_client ("delete_task", "task",
                                               delete_task_data->task_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("delete_task", "Permission denied"));
                log_event_fail ("task", "Task", delete_task_data->task_id,
                                "deleted");
                break;
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "delete_task", "A task_id attribute is required"));
      ...
      case CLIENT_DELETE_USER:
        if (delete_user_data->user_id || delete_user_data->name)
          switch (delete_user (
            delete_user_data->user_id, delete_user_data->name,
            delete_user_data->ultimate, 1, delete_user_data->inheritor_id,
            delete_user_data->inheritor_name))
            {
            case 0:
              SEND_TO_CLIENT_OR_FAIL (XML_OK ("delete_user"));
              log_event ("user", "User", delete_user_data->user_id, "deleted");
              break;
            case 2:
              if (send_find_error_to_client ("delete_user", "user",
                                             delete_user_data->user_id
                                               ? delete_user_data->user_id
                                               : delete_user_data->name,
                                             gmp_parser))
            ...
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "delete_user", "Attempt to delete a predefined"
                               " user"));
              break;
            case 4:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_user", "User has an active task"));
              break;
            case 5:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "delete_user", "Attempt to delete current user"));
              break;
            case 6:
            ...
            case 7:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "delete_user", "Inheritor is the same as the deleted"
                               " user."));
              break;
            case 8:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_user", "Invalid inheritor."));
              break;
            case 9:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "delete_user", "Resources owned by the user are still"
                               " in use by others."));
              break;
            case 10:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_user", "User is Feed Import Owner"));
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("delete_user", "Permission denied"));
              log_event_fail ("user", "User", delete_user_data->user_id,
                              "deleted");
              break;
            default:
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "delete_user", "A user_id attribute is required"));
      ...
      case CLIENT_DESCRIBE_AUTH:
        {
          if (acl_user_may ("describe_auth") == 0)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("describe_auth", "Permission denied"));
              set_client_state (CLIENT_AUTHENTIC);
              break;
            }
          ...
          if (gvm_auth_ldap_enabled ())
            {
              ...
              SENDF_TO_CLIENT_OR_FAIL ("<group name=\"method:ldap_connect\">"
                                       "<auth_conf_setting>"
                                       "<key>enable</key>"
                                       "<value>%s</value>"
                                       "</auth_conf_setting>"
              ...
              if (ldap_cacert)
                {
                  ...
                  SENDF_TO_CLIENT_OR_FAIL ("<auth_conf_setting>"
                                           "<key>cacert</key>"
                                           "<value>%s</value>",
                                           ldap_cacert);

                  get_certificate_info (ldap_cacert, -1, &activation_time,
                                        &expiration_time, &md5_fingerprint,
                                        NULL,          /* sha256_fingerprint */
                                        NULL,          /* subject */
                                        &issuer, NULL, /* serial */
                                        NULL);         /* certificate_format */
          ...
          if (gvm_auth_radius_enabled ())
            {
              ...
              manage_get_radius_info (&radius_enabled, &radius_host,
                                      &radius_key);
              SENDF_TO_CLIENT_OR_FAIL ("<group name=\"method:radius_connect\">"
                                       "<auth_conf_setting>"
                                       "<key>enable</key>"
                                       "<value>%s</value>"
                                       "</auth_conf_setting>"
      ...
      case CLIENT_HELP:
        if (acl_user_may ("help") == 0)
          {
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("help", "Permission denied"));
        ...
        if (help_data->format == NULL
            ...
            while ((*commands).name)
              {
                if (command_disabled (gmp_parser, (*commands).name) == 0)
                  {
                    int count;
                    SENDF_TO_CLIENT_OR_FAIL ("    %s", (*commands).name);
                    for (count = 23 - strlen ((*commands).name); count > 0;
                         count--)
                      SEND_TO_CLIENT_OR_FAIL (" ");
        ...
        else
          {
            ...
            switch (manage_schema (help_data->format, &output, &output_len,
              ...
              case 0:
                {
                  SENDF_TO_CLIENT_OR_FAIL (
                    ...
                    " extension=\"%s\""
                    " content_type=\"%s\">",
                    help_data->format ? help_data->format : "XML", extension,
                    content_type);
                  g_free (extension);
                  g_free (content_type);
                  ...
                  if (output && strlen (output))
                    {
                      ...
                      if (help_data->format
                          && strcasecmp (help_data->format, "XML"))
                        {
                          ...
                          base64 =
                            g_base64_encode ((guchar *) output, output_len);
                          if (send_to_client (base64, write_to_client,
                      ...
                      else
                        {
                          ...
                          if (send_to_client (output, write_to_client,
                                              write_to_client_data))
                            {
                              g_free (output);
                              error_send_to_client (error);
      ...
      case CLIENT_CREATE_ASSET:
        {
          ...
          if (create_asset_data->report_id == NULL
            ...
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_asset", "A report ID or an"
            ...
            switch (create_asset_report (create_asset_data->report_id,
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_asset", "Asset exists already"));
                log_event_fail ("asset", "Asset", NULL, "created");
                break;
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_asset", "Name may only contain alphanumeric"
                                  " characters"));
                log_event_fail ("asset", "Asset", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_asset", "Permission denied"));
                log_event_fail ("asset", "Asset", NULL, "created");
                break;
              default:
                assert (0);
              case -1:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_asset"));
                log_event_fail ("asset", "Asset", NULL, "created");
                break;
              }
          else if (strcasecmp (create_asset_data->type, "host") == 0)
            {
              switch (create_asset_host (create_asset_data->name,
                                         create_asset_data->comment, &asset))
                {
                case 0:
                  {
                    char *uuid;
                ...
                case 2:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_asset", "Name must be an IP address"));
                  log_event_fail ("asset", "Asset", NULL, "created");
                  break;
                case 99:
                  SEND_TO_CLIENT_OR_FAIL (
            ...
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("create_asset", "ASSET TYPE must be 'host'"));
              log_event_fail ("asset", "Asset", NULL, "created");
              break;
            }
      ...
      case CLIENT_CREATE_ALERT:
        {
          ...
          if (create_alert_data->copy)
            switch (copy_alert (create_alert_data->name,
                                ...
                                create_alert_data->copy, &new_alert))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_alert", "Alert exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_alert", "Permission denied"));
                log_event_fail ("alert", "Alert", NULL, "created");
                break;
              case -1:
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_alert"));
                log_event_fail ("alert", "Alert", NULL, "created");
                break;
              }
          else if (strlen (create_alert_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_alert", "A NAME element which"
                              " is at least one character long is required"));
          else if (strlen (create_alert_data->condition) == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_alert", "A value in a"
                              " CONDITION element is required"));
          else if (strlen (create_alert_data->event) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_alert", "A value in an"
                                                " EVENT element is required"));
          else if (strlen (create_alert_data->method) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_alert", "A value in a"
                                                " METHOD element is required"));
          else if ((condition =
                      alert_condition_from_name (create_alert_data->condition))
                   == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_alert", "Failed to recognise condition name"));
          else if ((event = event_from_name (create_alert_data->event)) == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_alert", "Failed to recognise event name"));
          else if ((method = alert_method_from_name (create_alert_data->method))
                   == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_alert", "Failed to recognise method name"));
          else
            {
              switch (create_alert (
                create_alert_data->name, create_alert_data->comment,
                create_alert_data->filter_id, create_alert_data->active, event,
                create_alert_data->event_data, condition,
                create_alert_data->condition_data, method,
                create_alert_data->method_data, &new_alert))
                {
                case 0:
                  {
                    char *uuid;
                ...
                case 2:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Validation of email address failed"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 3:
                ...
                case 4:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Filter type must be result if"
                                    " specified"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 5:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Invalid or unexpected condition data"
                                    " name"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 6:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Syntax error in condition data"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 7:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Email subject too long"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 8:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Email message too long"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 9:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Failed to find filter for condition"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 12:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_alert", "Error in Send host"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 13:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_alert", "Error in Send port"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 14:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Failed to find report format for Send"
                                    " method"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 15:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_alert", "Error in SCP host"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 17:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Failed to find report format for SCP"
                                    " method"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 18:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in SCP credential"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 19:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_alert", "Error in SCP path"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 20:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Method does not match event type"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 21:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Condition does not match event type"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 31:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Unexpected event data name"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 32:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Syntax error in event data"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 40:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in SMB credential"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 41:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in SMB share path"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 42:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in SMB file path"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 43:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "SMB file path must not contain"
                ...
                case 50:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in TippingPoint credential"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 51:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in TippingPoint hostname"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 52:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Error in TippingPoint TLS"
                                    " certificate"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 53:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "TippingPoint TLS workaround must be"
                                    " set to 0 or 1"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 60:
                ...
                case 61:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Email recipient credential must have"
                                    " type 'pgp' or 'smime'"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 70:
                ...
                case 71:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "vFire credential must have"
                                    " type 'up'"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 80:
                ...
                case 81:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_alert", "Sourcefire credential must have"
                                    " type 'pw' or 'up'"));
                  log_event_fail ("alert", "Alert", NULL, "created");
                  break;
                case 99:
      ...
      case CLIENT_CREATE_ALERT_CONDITION_DATA:
        {
          ...
          string = g_strconcat (create_alert_data->part_name, "0",
                                create_alert_data->part_data, NULL);
      ...
      case CLIENT_CREATE_CREDENTIAL:
        {
          ...
          if (create_credential_data->copy)
            switch (copy_credential (
              create_credential_data->name, create_credential_data->comment,
              create_credential_data->copy, &new_credential))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Credential exists already"));
                log_event_fail ("credential", "Credential", NULL, "created");
                break;
              case 2:
                if (send_find_error_to_client (
                      "create_credential", "credential",
                      create_credential_data->copy, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_credential", "Permission denied"));
                log_event_fail ("credential", "Credential", NULL, "created");
                break;
              case -1:
              default:
            ...
            {
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("create_credential",
            ...
            {
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "create_credential", "Erroneous Private Key."));
            }
          else if (create_credential_data->key
            ...
            {
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "create_credential", "Erroneous Public Key."));
            }
            ...
            {
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "create_credential", "Erroneous Certificate."));
            }
          else
            switch (create_credential (
              create_credential_data->name, create_credential_data->comment,
              ...
              create_credential_data->type,
              create_credential_data->allow_insecure, &new_credential))
              {
              case 0:
                {
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Login may only contain alphanumeric"
                                       " characters or the following:"
                                       " - _ \\ . @"));
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Erroneous private key or associated"
                                       " passphrase"));
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Erroneous credential type"));
                break;
              case 5:
                SEND_TO_CLIENT_OR_FAIL (
              ...
              case 6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type requires a password"));
                break;
              case 7:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type requires a private key"));
                break;
              case 8:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type requires a certificate"));
                break;
              case 9:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type requires a public key"));
                break;
              case 10:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type cannot be generated"
                                       " automatically"));
                break;
              case 11:
                SEND_TO_CLIENT_OR_FAIL (
              ...
              case 12:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "Selected type requires an"
                                       " auth_algorithm"));
                break;
              case 14:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              ...
              case 16:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_credential", "privacy algorithm must be 'aes', 'des'"
                                       " or empty"));
                break;
              case 17:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
      ...
      case CLIENT_CREATE_FILTER:
        {
          ...
          if (create_filter_data->copy)
            switch (copy_filter (create_filter_data->name,
                                 ...
                                 create_filter_data->copy, &new_filter))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_filter", "Filter exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_filter", "Permission denied"));
                log_event_fail ("filter", "Filter", NULL, "created");
                break;
              case -1:
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_filter"));
                log_event_fail ("filter", "Filter", NULL, "created");
                break;
              }
          else if (create_filter_data->name == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_filter", "A NAME is required"));
          else if (strlen (create_filter_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_filter", "Name must be at"
                                                 " least one character long"));
          else
            switch (create_filter (
              create_filter_data->name, create_filter_data->comment,
              create_filter_data->type, create_filter_data->term, &new_filter))
              {
              case 0:
                {
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_filter", "Type must be a valid GMP type"));
                log_event_fail ("filter", "Filter", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_GROUP:
        {
          ...
          if (create_group_data->copy)
            switch (copy_group (create_group_data->name,
                                ...
                                create_group_data->copy, &new_group))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_group", "Group exists already"));
              ...
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_group", "Syntax error in group name"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_group", "Permission denied"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              case -1:
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_group"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              }
          else if (create_group_data->name == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_group", "A NAME is required"));
          else if (strlen (create_group_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_group", "Name must be at"
                                                " least one character long"));
          else
            switch (create_group (create_group_data->name,
                                  create_group_data->comment,
                                  create_group_data->users,
                                  create_group_data->special_full, &new_group))
              {
              case 0:
                {
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_group", "Failed to find user"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_group", "Error in user name"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_NOTE:
        {
          ...
          if (create_note_data->copy)
            switch (copy_note (create_note_data->copy, &new_note))
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_note", "Note exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_note", "Permission denied"));
                log_event_fail ("note", "Note", NULL, "created");
                break;
              case -1:
              default:
            ...
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_note", "An NVT entity is required"));
          else if (create_note_data->text == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_note", "A TEXT entity is required"));
          else if (create_note_data->hosts
                   && ((max =
                          manage_count_hosts (create_note_data->hosts, NULL))
                       == -1))
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_note", "Error in host specification"));
          else if (create_note_data->hosts && (max > manage_max_hosts ()))
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_note", "Host specification exceeds maximum number of"
                             " hosts"));
            ...
            switch (
              create_note (create_note_data->active, create_note_data->nvt_oid,
                           create_note_data->text, create_note_data->hosts,
                           create_note_data->port, create_note_data->severity,
                           create_note_data->threat, task, result, &new_note))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_note", "Error in port specification"));
                log_event_fail ("note", "Note", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_OVERRIDE:
        {
          ...
          if (create_override_data->copy)
            switch (copy_override (create_override_data->copy, &new_override))
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_override", "Override exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_override", "Permission denied"));
                log_event_fail ("override", "Override", NULL, "created");
                break;
              case -1:
              default:
                                             ...
                                             gmp_parser))
                {
                  ...
                  return;
                   ...
                   && find_result_with_permission (
                     create_override_data->result_id, &result, NULL))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_override"));
            ...
            switch (create_override (
              create_override_data->active, create_override_data->nvt_oid,
              create_override_data->text, create_override_data->hosts,
              create_override_data->port, create_override_data->threat,
              create_override_data->new_threat, create_override_data->severity,
              create_override_data->new_severity, task, result, &new_override))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_override", "Error in port specification"));
                log_event_fail ("override", "Override", NULL, "created");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_override", "Error in new_severity specification"));
                log_event_fail ("override", "Override", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_PERMISSION:
        {
          ...
          if (create_permission_data->copy)
            switch (copy_permission (create_permission_data->comment,
              ...
              case 0:
                {
                  ...
                  SENDF_TO_CLIENT_OR_FAIL (
                    XML_OK_CREATED_ID ("create_permission"), uuid);
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_permission", "Permission exists already"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case 2:
                if (send_find_error_to_client (
                      "create_permission", "permission",
                      create_permission_data->copy, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_permission", "Permission denied"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case -1:
              default:
            ...
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_permission", "A NAME is required"));
          else if (strlen (create_permission_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_permission", "Name must be at"
                                   " least one character long"));
          else
            switch (create_permission (
              create_permission_data->name, create_permission_data->comment,
              create_permission_data->resource_type,
              create_permission_data->resource_id,
              create_permission_data->subject_type,
              create_permission_data->subject_id, &new_permission))
              {
              case 0:
                {
              ...
              case 5:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_permission", "Error in RESOURCE"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case 6:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_permission", "Error in SUBJECT"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case 7:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_permission", "Error in NAME"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case 8:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_permission", "Attempt to create permission on"
                                       " permission"));
                log_event_fail ("permission", "Permission", NULL, "created");
                break;
              case 9:
      ...
      case CLIENT_CREATE_PORT_RANGE:
        {
          ...
          if (create_port_range_data->start == NULL
            ...
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_port_range", "A START, END and"
                                   " PORT_LIST ID are required"));
          else
            switch (create_port_range (
              create_port_range_data->port_list_id,
              create_port_range_data->type, create_port_range_data->start,
              create_port_range_data->end, create_port_range_data->comment,
              &new_port_range))
              {
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_port_range", "Port range START must be a number"
                                       " 1-65535"));
                log_event_fail ("port_range", "Port Range", NULL, "created");
                break;
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_port_range", "Port range END must be a number"
                                       " 1-65535"));
                log_event_fail ("port_range", "Port Range", NULL, "created");
                break;
              case 3:
              ...
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_port_range", "Port range TYPE must be TCP or UDP"));
                log_event_fail ("port_range", "Port Range", NULL, "created");
                break;
              case 5:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_port_range", "Port list is in use"));
                break;
              case 6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_port_range", "New range overlaps an existing"
                                       " range"));
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_port_range", "Permission denied"));
                break;
              case -1:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_REPORT:
        {
          ...
          if (create_report_data->results == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_report", "A REPORT element is required"));
          else if (create_report_data->type
                   && strcmp (create_report_data->type, "scan"))
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_report", "Type must be 'scan'"));
          else
            switch (create_report (
              create_report_data->results, create_report_data->task_id,
              create_report_data->in_assets, create_report_data->scan_start,
              create_report_data->scan_end, create_report_data->host_starts,
              create_report_data->host_ends, create_report_data->details,
              &uuid))
              {
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_report", "Permission denied"));
                log_event_fail ("report", "Report", NULL, "created");
                break;
              case -1:
              case -2:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_report"));
                log_event_fail ("report", "Report", NULL, "created");
                break;
              case -3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_report", "A TASK id attribute is required"));
                log_event_fail ("report", "Report", NULL, "created");
                break;
              case -4:
                log_event_fail ("report", "Report", NULL, "created");
                if (send_find_error_to_client ("create_report", "task",
                                               create_report_data->task_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case -6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_report", "Permission to add to Assets denied"));
                log_event_fail ("report", "Report", NULL, "created");
                break;
              default:
                {
                  SENDF_TO_CLIENT_OR_FAIL (XML_OK_CREATED_ID ("create_report"),
                                           uuid);
      ...
      case CLIENT_CREATE_REPORT_RR_RESULTS_RESULT:
        ...
        CLOSE (CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_DETECTION_RESULT,
               DETAILS);
        CLOSE (CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_DETECTION_RESULT_DETAILS,
               DETAIL);
        CLOSE (
          CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_DETECTION_RESULT_DETAILS_DETAIL,
          NAME);
      ...
      case CLIENT_CREATE_ROLE:
        {
          ...
          if (create_role_data->copy)
            switch (copy_role (create_role_data->name,
                               ...
                               create_role_data->copy, &new_role))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_role", "Role exists already"));
              ...
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_role", "Syntax error in role name"));
                log_event_fail ("role", "Role", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_role", "Permission denied"));
                log_event_fail ("role", "Role", NULL, "created");
                break;
              case -1:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_role"));
                log_event_fail ("role", "Role", NULL, "created");
                break;
              }
          else if (create_role_data->name == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_role", "A NAME is required"));
          else if (strlen (create_role_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_role", "Name must be at"
                                               " least one character long"));
          else
            switch (create_role (create_role_data->name,
                                 create_role_data->comment,
                                 create_role_data->users, &new_role))
              {
              case 0:
                {
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_role", "Failed to find user"));
                log_event_fail ("role", "Role", NULL, "created");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_role", "Error in user name"));
                log_event_fail ("group", "Group", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_TAG:
        {
          ...
          if (create_tag_data->copy)
            switch (copy_tag (create_tag_data->name, create_tag_data->comment,
                              create_tag_data->copy, &new_tag))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_tag", "Tag exists already"));
                log_event_fail ("tag", "Tag", NULL, "created");
                break;
              case 2:
                if (send_find_error_to_client (
                      "create_tag", "tag", create_tag_data->copy, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_tag", "Permission denied"));
                log_event_fail ("tag", "Tag", NULL, "created");
                break;
              case -1:
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_tag"));
                log_event_fail ("tag", "Tag", NULL, "created");
                break;
              }
          else if (create_tag_data->name == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_tag", "A NAME element is required"));
          else if (strlen (create_tag_data->name) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_tag", "Name must be"
                                              " at least one character long"));
          else if (create_tag_data->resource_ids == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "create_tag", "A RESOURCES element with TYPE element"
                            " is required"));
          else if (create_tag_data->resource_type == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("create_tag",
                                                      "RESOURCES requires"
                                                      " a TYPE element"));
          else if (valid_db_resource_type (create_tag_data->resource_type) == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_tag", "TYPE in RESOURCES must be"
                                              " a valid resource type."));
          else if (strcasecmp (create_tag_data->resource_type, "tag") == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_tag", "TYPE in RESOURCES must not"
                                              " be 'tag'."));
          else
            {
              gchar *error_extra = NULL;
              switch (create_tag (
                create_tag_data->name, create_tag_data->comment,
                create_tag_data->value, create_tag_data->resource_type,
                create_tag_data->resource_ids,
                create_tag_data->resources_filter, create_tag_data->active,
                &new_tag, &error_extra))
                {
                case 0:
                  {
                ...
                case 3:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_tag", "Too many resources selected"));
                  log_event_fail ("tag", "Tag", NULL, "created");
                  break;
                case 99:
                  SEND_TO_CLIENT_OR_FAIL (
      ...
      case CLIENT_CREATE_TARGET:
        {
          ...
          if (create_target_data->copy)
            switch (copy_target (create_target_data->name,
                                 ...
                                 create_target_data->copy, &new_target))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Target exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Permission denied"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case -1:
              default:
              ...
              if (send_find_error_to_client (
                    ...
                    gmp_parser))
                {
                  ...
                  return;
                   ...
                   && find_credential_with_permission (
                     create_target_data->smb_credential_id, &smb_credential,
                     "get_credentials"))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_target"));
          else if (create_target_data->smb_credential_id == NULL
                   && create_target_data->smb_lsc_credential_id
                   && find_credential_with_permission (
                     create_target_data->smb_lsc_credential_id, &smb_credential,
                     "get_credentials"))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_target"));
          else if ((create_target_data->smb_credential_id
                    || create_target_data->smb_lsc_credential_id)
              ...
              if (send_find_error_to_client (
                    ...
                    gmp_parser))
                {
                  ...
                  return;
                   ...
                   && find_credential_with_permission (
                     create_target_data->esxi_credential_id, &esxi_credential,
                     "get_credentials"))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_target"));
                   ...
                   && find_credential_with_permission (
                     create_target_data->esxi_lsc_credential_id,
                     &esxi_credential, "get_credentials"))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_target"));
          else if ((create_target_data->esxi_credential_id
                    || create_target_data->esxi_lsc_credential_id)
                   && esxi_credential == 0)
              ...
              if (send_find_error_to_client (
                    ...
                    gmp_parser))
                {
                  ...
                  return;
                   ...
                   && find_credential_with_permission (
                     create_target_data->snmp_credential_id, &snmp_credential,
                     "get_credentials"))
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_target"));
          else if (create_target_data->snmp_credential_id
                   && snmp_credential == 0)
            {
              if (send_find_error_to_client (
                    "create_target", "Credential",
                    create_target_data->snmp_credential_id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
          ...
          /* Create target from host string. */
            ...
            switch (create_target (
              create_target_data->name, create_target_data->asset_hosts_filter,
              create_target_data->hosts, create_target_data->exclude_hosts,
              create_target_data->comment, create_target_data->port_list_id,
              create_target_data->port_range, ssh_credential,
              create_target_data->ssh_credential_id
              ...
              create_target_data->allow_simultaneous_ips, &new_target))
              {
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Target exists already"));
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "Error in host specification"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "Host specification exceeds maximum number"
                                   " of hosts"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Error in port range"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 5:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Error in SSH port"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 6:
                log_event_fail ("target", "Target", NULL, "created");
                if (send_find_error_to_client ("create_target", "port_list",
                                               create_target_data->port_list_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 7:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_target", "Error in alive test"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 8:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "SSH credential must be of type"
                                   " 'up' or 'usk'"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 9:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "SMB credential must be of type"
                                   " 'up'"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 10:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "ESXi credential must be of type"
                                   " 'up'"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 11:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "SNMP credential must be of type"
                                   " 'snmp'"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 12:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_target", "One of PORT_LIST and PORT_RANGE are"
                                   " required"));
                log_event_fail ("target", "Target", NULL, "created");
                break;
              case 99:
      ...
      case CLIENT_CREATE_TASK:
        {
          ...
          if (create_task_data->copy)
            {
              ...
              ret = copy_task (name, comment, create_task_data->copy,
              ...
              switch (ret)
                ...
                case 1:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_task", "Task exists already"));
                ...
                case 99:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_task", "Permission denied"));
                  log_event_fail ("task", "Task", NULL, "created");
                  break;
                case -1:
          ...
          if (create_task_data->name == NULL)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("create_task", "A NAME is required"));
              goto create_task_fail;
            }
          else
          ...
          if (create_task_data->target_id == NULL)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("create_task", "A target is required"));
              goto create_task_fail;
            }
          ...
          if (create_task_data->config_id == NULL)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("create_task", "A config is required"));
              goto create_task_fail;
            }
          ...
          while (index--)
            {
              ...
              alert_id =
                (gchar *) g_ptr_array_index (create_task_data->alerts, index);
              ...
              if (alert == 0)
                {
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("create_task", "Alert must exist"));
                  goto create_task_fail;
                }
          ...
          if (create_task_data->observers)
            {
              ...
              switch (fail)
                ...
                case 2:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_task", "User name error in observers"));
                  goto create_task_fail;
                case -1:
                default:
                  SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_task"));
          ...
          if (create_task_data->groups->len)
            {
              ...
              switch ((fail = set_task_groups (create_task_data->task,
                ...
                case 0:
                  break;
                case 1:
                  if (send_find_error_to_client ("create_task", "group",
          ...
          if ((scanner == 0) || (scanner_type (scanner) != SCANNER_TYPE_CVE))
            {
              ...
              if (!create_task_check_config_scanner (config, scanner))
                {
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "create_task", "Scanner and config mismatched types."));
                  goto create_task_fail;
                }
            }
          if (find_target_with_permission (create_task_data->target_id, &target,
                                           "get_targets"))
            {
              SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("create_task"));
              goto create_task_fail;
          ...
          if (create_task_data->preferences)
            switch (set_task_preferences (create_task_data->task,
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_task", "Invalid auto_delete value"));
                goto create_task_fail;
              case 2:
                SENDF_TO_CLIENT_OR_FAIL (
          ...
          SENDF_TO_CLIENT_OR_FAIL (XML_OK_CREATED_ID ("create_task"), tsk_uuid);
          make_task_complete (create_task_data->task);
          log_event ("task", "Task", tsk_uuid, "created");
          g_free (tsk_uuid);
          create_task_data_reset (create_task_data);
      ...
      case CLIENT_CREATE_TASK_PREFERENCES_PREFERENCE:
        array_add (create_task_data->preferences, create_task_data->preference);
      ...
      case CLIENT_CREATE_USER:
        {
          ...
          if (create_user_data->copy)
            switch (copy_user (create_user_data->name, NULL,
                               create_user_data->copy, &new_user))
              {
              case 0:
                {
                  char *uuid;
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_user", "User exists already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_user", "Permission denied"));
                log_event_fail ("user", "User", NULL, "created");
                break;
              case -1:
              default:
            ...
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("create_user", "A name is required"));
          else
            switch (create_user (
              create_user_data->name,
              create_user_data->password ? create_user_data->password : "",
              create_user_data->comment ? create_user_data->comment : "",
              create_user_data->hosts, create_user_data->hosts_allow,
              create_user_data->ifaces, create_user_data->ifaces_allow,
              create_user_data->sources, create_user_data->groups,
              &fail_group_id, create_user_data->roles, &fail_role_id, &errdesc,
              &new_user, 1))
              {
              case 0:
                {
              ...
              case 1:
                if (send_find_error_to_client ("create_user", "group",
                                               fail_group_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                ...
                break;
              case 2:
                if (send_find_error_to_client ("create_user", "role",
                                               fail_role_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "create_user", "Error in host specification"));
                log_event_fail ("user", "User", NULL, "created");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
              ...
              case -2:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_user", "User already exists"));
                log_event_fail ("user", "User", NULL, "created");
                break;
              case -3:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_user", "Error in SOURCE"));
                log_event_fail ("user", "User", NULL, "created");
                break;
              case -1:
                if (errdesc)
      ...
      case CLIENT_EMPTY_TRASHCAN:
        switch (manage_empty_trashcan ())
          ...
          case 99:
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("empty_trashcan", "Permission denied"));
            break;
          default: /* Programming error. */
            assert (0);
          case -1:
      ...
      case CLIENT_MODIFY_ALERT:
        {
          ...
          if (strlen (modify_alert_data->event)
            ...
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_alert", "Failed to recognise event name"));
          else if (strlen (modify_alert_data->condition)
                   && (condition = alert_condition_from_name (
                         modify_alert_data->condition))
                        == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_alert", "Failed to recognise condition name"));
          else if (strlen (modify_alert_data->method)
                   && (method =
                         alert_method_from_name (modify_alert_data->method))
                        == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_alert", "Failed to recognise method name"));
          else
            switch (modify_alert (
              modify_alert_data->alert_id, modify_alert_data->name,
              modify_alert_data->comment, modify_alert_data->filter_id,
              modify_alert_data->active, event, modify_alert_data->event_data,
              condition, modify_alert_data->condition_data, method,
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "alert with new name exists already"));
                log_event_fail ("alert", "Alert", modify_alert_data->alert_id,
                                "modified");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "An alert_id is required"));
                log_event_fail ("alert", "Alert", modify_alert_data->alert_id,
                                "modified");
                break;
              case 4:
              ...
              case 5:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Filter type must be result if"
                                  " specified"));
                log_event_fail ("alert", "Alert", modify_alert_data->alert_id,
                                "modified");
                break;
              case 6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Validation of email address failed"));
                log_event_fail ("alert", "Alert", modify_alert_data->alert_id,
                                "modified");
                break;
              case 7:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Invalid or unexpected condition data"
                                  " name"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 8:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Syntax error in condition data"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 9:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Email subject too long"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 10:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Email message too long"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 11:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Failed to find filter for condition"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 12:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in Send host"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 13:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in Send port"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 14:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Failed to find report format for Send"
                                  " method"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 15:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SCP host"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 17:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Failed to find report format for SCP"
                                  " method"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 18:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SCP credential"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 19:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SCP path"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 20:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Method does not match event type"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 21:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Condition does not match event type"));
                log_event_fail ("alert", "Alert", NULL, "modify");
                break;
              case 31:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Unexpected event data name"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 32:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "Syntax error in event data"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 40:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SMB credential"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 41:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SMB share path"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 42:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Error in SMB file path"));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              case 43:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_alert", "SMB file path must not contain"
                                  " any file or subdirectory ending in"
                                  " a dot (.)."));
                log_event_fail ("alert", "Alert", NULL, "modified");
                break;
              ...
              case 52:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_alert", "Error in TippingPoint TLS"
                                                    " certificate"));
                log_event_fail ("alert", "Alert", NULL, "created");
                break;
              case 53:
              ...
              case 71:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("create_alert", "vFire credential must have"
                                                    " type 'up'"));
                log_event_fail ("alert", "Alert", NULL, "created");
                break;
              case 80:
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_alert", "Permission denied"));
                log_event_fail ("alert", "Alert", modify_alert_data->alert_id,
                                "modified");
                break;
              default:
      ...
      case CLIENT_MODIFY_ALERT_EVENT_DATA:
        {
          ...
          string = g_strconcat (modify_alert_data->part_name, "0",
                                modify_alert_data->part_data, NULL);
      ...
      case CLIENT_MODIFY_ASSET:
        {
          switch (modify_asset (modify_asset_data->asset_id,
            ...
            case 2:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_asset", "asset with new name exists already"));
              log_event_fail ("asset", "Asset", modify_asset_data->asset_id,
                              "modified");
              break;
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_asset", "MODIFY_asset requires a asset_id"));
              log_event_fail ("asset", "Asset", modify_asset_data->asset_id,
                              "modified");
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_asset", "Permission denied"));
              log_event_fail ("asset", "Asset", modify_asset_data->asset_id,
                              "modified");
              break;
            default:
      ...
      case CLIENT_MODIFY_AUTH:
        {
          ...
          if (acl_user_may ("modify_auth") == 0)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_auth", "Permission denied"));
          ...
          while (item)
            {
              ...
              if (group == NULL)
                {
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_auth", "GROUP requires a name attribute"));
                  set_client_state (CLIENT_AUTHENTIC);
                  modify_auth_data_reset (modify_auth_data);
                  break;
                }
      ...
      case CLIENT_MODIFY_CREDENTIAL:
        {
          switch (modify_credential (
            modify_credential_data->credential_id, modify_credential_data->name,
            modify_credential_data->comment, modify_credential_data->login,
            ...
            case 0:
              ...
              log_event ("credential", "Credential",
                         modify_credential_data->credential_id, "modified");
              break;
            case 1:
              if (send_find_error_to_client (
                    "modify_credential", "credential",
                    modify_credential_data->credential_id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
            ...
            case 2:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "credential with new name"
                                     " exists already"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
            ...
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "A credential_id is required"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 4:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Login name must not be empty and may"
            ...
            case 5:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Invalid or empty certificate"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 6:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Invalid or empty auth_algorithm"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 7:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Invalid or empty privacy_algorithm"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 8:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Invalid or empty private key"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 9:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Invalid or empty public key"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
            case 10:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_credential", "Privacy password must also be empty"
            ...
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_credential", "Permission denied"));
              log_event_fail ("credential", "Credential",
                              modify_credential_data->credential_id,
                              "modified");
              break;
      ...
      case CLIENT_MODIFY_FILTER:
        {
          switch (modify_filter (
            modify_filter_data->filter_id, modify_filter_data->name,
            modify_filter_data->comment, modify_filter_data->term,
            ...
            case 1:
              ...
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            case 2:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_filter", "Filter with new name exists already"));
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            case 3:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_filter", "Error in type name"));
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            case 4:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_filter", "A filter_id is required"));
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            case 5:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_filter", "Filter is used by an alert so type must be"
                                 " 'result' if specified"));
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            ...
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_filter", "Permission denied"));
              log_event_fail ("filter", "Filter", modify_filter_data->filter_id,
                              "modified");
              break;
            default:
      ...
      case CLIENT_MODIFY_GROUP:
        {
          switch (
            modify_group (modify_group_data->group_id, modify_group_data->name,
                          modify_group_data->comment, modify_group_data->users))
            {
            case 0:
            ...
            case 1:
              ...
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            case 2:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_group", "Failed to find user"));
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_group", "A group_id attribute is required"));
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            case 4:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_group", "Error in user name"));
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            case 5:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_group", "Group with new name exists already"));
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_group", "Permission denied"));
              log_event_fail ("group", "Group", modify_group_data->group_id,
                              "modified");
              break;
            default:
      ...
      case CLIENT_MODIFY_NOTE:
        {
          if (acl_user_may ("modify_note") == 0)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_note", "Permission denied"));
          ...
          if (modify_note_data->note_id == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_note", "A note_id attribute is required"));
          else if (modify_note_data->text == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_note", "A TEXT entity is required"));
          else
            switch (modify_note (
              modify_note_data->note_id, modify_note_data->active,
              modify_note_data->nvt_oid, modify_note_data->text,
              modify_note_data->hosts, modify_note_data->port,
              modify_note_data->severity, modify_note_data->threat,
              modify_note_data->task_id, modify_note_data->result_id))
              {
              case 0:
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_note", "Error in port specification"));
                log_event_fail ("note", "Note", modify_note_data->note_id,
                                "modified");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_note", "Error in severity specification"));
                log_event_fail ("note", "Note", modify_note_data->note_id,
                                "modified");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_note", "Invalid nvt oid"));
                log_event_fail ("note", "Note", modify_note_data->note_id,
                                "modified");
                break;
              case 5:
      ...
      case CLIENT_MODIFY_OVERRIDE:
        {
          ...
          if (acl_user_may ("modify_override") == 0)
            {
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_override", "Permission denied"));
          ...
          if (modify_override_data->override_id == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_override", "An override_id attribute is required"));
          else if (modify_override_data->text == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_override", "A TEXT entity is required"));
          else if (modify_override_data->hosts
                   && ((max = manage_count_hosts (modify_override_data->hosts,
                                                  NULL))
                       == -1))
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_override", "Error in host specification"));
          else if (modify_override_data->hosts && (max > manage_max_hosts ()))
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_override", "Host specification exceeds maximum number"
                                 " of hosts"));
          else
            switch (modify_override (
              modify_override_data->override_id, modify_override_data->active,
              modify_override_data->nvt_oid, modify_override_data->text,
              modify_override_data->hosts, modify_override_data->port,
              modify_override_data->threat, modify_override_data->new_threat,
              modify_override_data->severity,
              modify_override_data->new_severity, modify_override_data->task_id,
              modify_override_data->result_id))
              {
              case 0:
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_override", "ACTIVE must be an integer >= -2"));
                break;
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_override", "Error in port specification"));
                log_event_fail ("override", "Override",
                                modify_override_data->override_id, "modified");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_override", "Error in severity specification"));
                log_event_fail ("override", "Override",
                                modify_override_data->override_id, "modified");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_override", "Invalid nvt oid"));
                log_event_fail ("override", "Override",
                                modify_override_data->override_id, "modified");
                break;
              case 5:
                if (send_find_error_to_client (
                      "modify_override", "override",
                      modify_override_data->override_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
      ...
      case CLIENT_MODIFY_PERMISSION:
        {
          if (modify_permission_data->permission_id == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_permission", "A permission_id attribute is required"));
          else
            switch (modify_permission (modify_permission_data->permission_id,
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_permission", "Permission exists already"));
              ...
              case 3:
                ...
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("modify_permission",
                                                          "A PERMISSION"
                                                          " ID is required"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
              ...
              case 5:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_permission", "Error in RESOURCE"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
                                "modified");
                break;
              case 6:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_permission", "Error in SUBJECT"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
                                "modified");
                break;
              case 7:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_permission", "Error in NAME"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
                                "modified");
                break;
              case 8:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_permission", "NAME required to find resource"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
                                "modified");
                break;
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_permission", "Permission denied"));
                log_event_fail ("permission", "Permission",
                                modify_permission_data->permission_id,
                                "modified");
                break;
      ...
      case CLIENT_MODIFY_PORT_LIST:
        {
          switch (modify_port_list (modify_port_list_data->port_list_id,
            ...
            case 1:
              if (send_find_error_to_client (
                    "modify_port_list", "port_list",
                    modify_port_list_data->port_list_id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
            ...
            case 2:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_port_list", "Port List with new name exists already"));
              log_event_fail ("port_list", "Port List",
                              modify_port_list_data->port_list_id, "modified");
              break;
            case 3:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_port_list", "A port_list_id is required"));
              log_event_fail ("port_list", "Port List",
                              modify_port_list_data->port_list_id, "modified");
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_port_list", "Permission denied"));
              log_event_fail ("port_list", "Port List",
                              modify_port_list_data->port_list_id, "modified");
              break;
            default:
      ...
      case CLIENT_MODIFY_REPORT_FORMAT:
        {
          switch (modify_report_format (
            modify_report_format_data->report_format_id,
            modify_report_format_data->name, modify_report_format_data->summary,
            ...
            case 1:
              if (send_find_error_to_client (
                    ...
                    modify_report_format_data->report_format_id, gmp_parser))
                {
                  error_send_to_client (error);
                  return;
                }
            ...
            case 2:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_report_format", "A report_format_id is required"));
              log_event_fail ("report_format", "Report Format",
                              modify_report_format_data->report_format_id,
                              "modified");
              break;
            ...
            case 4:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_report_format", "Parameter validation failed"));
              log_event_fail ("report_format", "Report Format",
                              modify_report_format_data->report_format_id,
                              "modified");
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_report_format", "Permission denied"));
              log_event_fail ("report_format", "Report Format",
                              modify_report_format_data->report_format_id,
                              "modified");
              break;
      ...
      case CLIENT_MODIFY_ROLE:
        {
          switch (
            modify_role (modify_role_data->role_id, modify_role_data->name,
                         modify_role_data->comment, modify_role_data->users))
            {
            case 0:
              SENDF_TO_CLIENT_OR_FAIL (XML_OK ("modify_role"));
              log_event ("role", "Role", modify_role_data->role_id, "modified");
              break;
            case 1:
              ...
              log_event_fail ("role", "Role", modify_role_data->role_id,
                              "modified");
              break;
            case 2:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_role", "Failed to find user"));
              log_event_fail ("role", "Role", modify_role_data->role_id,
                              "modified");
              break;
            case 3:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_role", "A role_id"
            ...
            case 4:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_role", "Error in user name"));
              log_event_fail ("role", "Role", modify_role_data->role_id,
                              "modified");
              break;
            case 5:
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_role", "Role with new name exists already"));
              log_event_fail ("role", "Role", modify_role_data->role_id,
                              "modified");
              break;
            case 99:
              SEND_TO_CLIENT_OR_FAIL (
                XML_ERROR_SYNTAX ("modify_role", "Permission denied"));
              log_event_fail ("role", "Role", modify_role_data->role_id,
                              "modified");
              break;
            default:
      ...
      case CLIENT_MODIFY_SETTING:
        {
          ...
          if (((modify_setting_data->name == NULL)
            ...
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_setting", "A NAME or setting_id"
                                                  " and a VALUE is required"));
          else
            switch (modify_setting (modify_setting_data->setting_id,
                                    modify_setting_data->name,
                                    modify_setting_data->value, &errdesc))
              {
              case 0:
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_setting", "Failed to find setting"));
                break;
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_setting", "Value validation failed"));
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_setting", "Permission denied"));
                break;
              case -1:
                if (errdesc)
                  {
                    char *buf =
                      make_xml_error_syntax ("modify_setting", errdesc);
                    SEND_TO_CLIENT_OR_FAIL (buf);
                    g_free (buf);
                    break;
                  }
                /* Fall through.  */
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("modify_setting"));
                break;
              }
          g_free (errdesc);
        }
      ...
      case CLIENT_MODIFY_TAG:
        {
          ...
          if (modify_tag_data->tag_id == NULL)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_tag", "A tag_id attribute is required"));
          else if (modify_tag_data->name
                   && strcmp (modify_tag_data->name, "") == 0)
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_tag", "name must be at least one"
                            " character long or omitted completely"));
          else if (modify_tag_data->resource_type
                   && valid_db_resource_type (modify_tag_data->resource_type)
                        == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_tag", "TYPE in RESOURCES must be"
                                              " a valid resource type."));
          else if (modify_tag_data->resource_type
                   && strcasecmp (modify_tag_data->resource_type, "tag") == 0)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_tag", "TYPE in RESOURCES must not"
                                              " be 'tag'."));
          else
            switch (modify_tag (
              modify_tag_data->tag_id, modify_tag_data->name,
              modify_tag_data->comment, modify_tag_data->value,
              modify_tag_data->resource_type, modify_tag_data->resource_ids,
              modify_tag_data->resources_filter,
              modify_tag_data->resources_action, modify_tag_data->active,
              &error_extra))
              {
              case 0:
                SENDF_TO_CLIENT_OR_FAIL (XML_OK ("modify_tag"));
                log_event ("tag", "Tag", modify_tag_data->tag_id, "modified");
                break;
              case 1:
                if (send_find_error_to_client (
                      "modify_tag", "tag", modify_tag_data->tag_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_tag", "A tag_id is required"));
                log_event_fail ("tag", "Tag", modify_tag_data->tag_id,
                                "modified");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_tag", "RESOURCES action must be"
                                                  " 'add', 'set', 'remove'"
                                                  " or empty."));
                log_event_fail ("tag", "Tag", modify_tag_data->tag_id,
                                "modified");
              ...
              case 6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_tag", "Too many resources selected"));
                log_event_fail ("tag", "Tag", NULL, "modified");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_tag", "Permission denied"));
                log_event_fail ("tag", "Tag", modify_tag_data->tag_id,
                                "modified");
                break;
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("modify_tag"));
                log_event_fail ("tag", "Tag", modify_tag_data->tag_id,
                                "modified");
                break;
              }
      ...
      case CLIENT_MODIFY_TARGET:
        {
          if (modify_target_data->target_id == NULL)
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_target", "A target_id"
                                                 " attribute is required"));
          else
            switch (modify_target (
              modify_target_data->target_id, modify_target_data->name,
              modify_target_data->hosts, modify_target_data->exclude_hosts,
              modify_target_data->comment, modify_target_data->port_list_id,
              modify_target_data->ssh_credential_id
                ? modify_target_data->ssh_credential_id
                : modify_target_data->ssh_lsc_credential_id,
              modify_target_data->ssh_credential_id
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Target exists already"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "Error in host specification"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "Host specification exceeds maximum number"
                                   " of hosts"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Error in port range"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 5:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Error in SSH port"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 6:
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                if (send_find_error_to_client ("modify_target", "port_list",
                                               modify_target_data->port_list_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 9:
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                if (send_find_error_to_client ("modify_target", "target",
                                               modify_target_data->target_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 10:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Error in alive test"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 11:
              ...
              case 12:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "EXCLUDE_HOSTS requires"
                                                     " a HOSTS"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 13:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("modify_target",
                                                          "HOSTS requires an"
                                                          " EXCLUDE_HOSTS"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
              ...
              case 15:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Target is in use"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 16:
              ...
              case 17:
                ...
                if (send_find_error_to_client (
                      "modify_target", "Credential",
                      modify_target_data->snmp_credential_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 18:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "SSH credential must be of type"
                                   " 'up' or 'usk'"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 19:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "SMB credential must be of type"
                                   " 'up'"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 20:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "ESXi credential must be of type"
                                   " 'up'"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 21:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "modify_target", "SNMP credential must be of type"
                                   " 'snmp'"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("modify_target", "Permission denied"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              case -1:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("modify_target"));
                log_event_fail ("target", "Target",
                                modify_target_data->target_id, "modified");
                break;
              default:
      ...
      case CLIENT_MODIFY_TASK:
        if (acl_user_may ("modify_task") == 0)
          {
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("modify_task", "Permission denied"));
        ...
        if (modify_task_data->task_id)
          {
            ...
            if (modify_task_data->action
                && (modify_task_data->comment || modify_task_data->alerts->len
                    || modify_task_data->groups->len || modify_task_data->name))
              SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                "modify_task", "Too many parameters at once"));
            else if (modify_task_data->action)
              {
                if (modify_task_data->file_name == NULL)
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_task", "FILE requires a name"
                                                     " attribute"));
                else if (strcmp (modify_task_data->action, "update") == 0)
                  {
                    switch (manage_task_update_file (
                      modify_task_data->task_id, modify_task_data->file_name,
                      modify_task_data->file ? modify_task_data->file : ""))
                      {
                      case 0:
                      ...
                      case 1:
                        if (send_find_error_to_client (
                              "modify_task", "Task", modify_task_data->task_id,
                              gmp_parser))
                          {
                            error_send_to_client (error);
                            return;
                  ...
                  {
                    switch (manage_task_remove_file (
                      modify_task_data->task_id, modify_task_data->file_name))
                      {
                      case 0:
                        log_event ("task", "Task", modify_task_data->task_id,
                ...
                else
                  {
                    SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                      "modify_task", "Action must be"
                                     " \"update\" or \"remove\""));
                    log_event_fail ("task", "Task", modify_task_data->task_id,
                                    "modified");
                  }
              }
            else
              switch (modify_task (
                modify_task_data->task_id, modify_task_data->name,
                modify_task_data->comment, modify_task_data->scanner_id,
                modify_task_data->target_id, modify_task_data->config_id,
                modify_task_data->observers, modify_task_data->alerts,
                modify_task_data->alterable, modify_task_data->groups,
                modify_task_data->schedule_id,
                modify_task_data->schedule_periods,
                modify_task_data->preferences, modify_task_data->hosts_ordering,
                &fail_alert_id, &fail_group_id))
                {
                case 0:
                  log_event ("task", "Task", modify_task_data->task_id,
                             "modified");
                ...
                case 2:
                  ...
                  break;
                case 3:
                  if (send_find_error_to_client ("modify_task", "scanner",
                                                 modify_task_data->scanner_id,
                                                 gmp_parser))
                    {
                      error_send_to_client (error);
                  ...
                  break;
                case 4:
                  if (send_find_error_to_client ("modify_task", "config",
                                                 modify_task_data->config_id,
                                                 gmp_parser))
                    {
                      error_send_to_client (error);
                ...
                case 5:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "Status must be New to edit config"));
                  break;
                case 6:
                case 7:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_task", "User name error"));
                  log_event_fail ("task", "Task", modify_task_data->task_id,
                                  "modified");
                  break;
                case 8:
                ...
                case 9:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "Task must be New to modify"
                                   " Alterable state"));
                  log_event_fail ("task", "Task", modify_task_data->task_id,
                                  "modified");
                  break;
                case 10:
                  ...
                  break;
                case 11:
                  if (send_find_error_to_client ("modify_task", "schedule",
                                                 modify_task_data->schedule_id,
                                                 gmp_parser))
                    {
                      error_send_to_client (error);
                  ...
                  break;
                case 12:
                  if (send_find_error_to_client ("modify_task", "target",
                                                 modify_task_data->target_id,
                                                 gmp_parser))
                    {
                      error_send_to_client (error);
                ...
                case 13:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "Invalid auto_delete value"));
                  log_event_fail ("task", "Task", modify_task_data->task_id,
                                  "modified");
                  break;
                case 14:
                ...
                case 15:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "Config and Scanner types mismatch"));
                  log_event_fail ("task", "Task", modify_task_data->task_id,
                                  "modified");
                  break;
                case 16:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "Status must be New to edit Target"));
                  log_event_fail ("task", "Task", modify_task_data->task_id,
                                  "modified");
                  break;
                case 17:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_task", "For container tasks only name, comment"
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "modify_task", "A task_id attribute is required"));
        modify_task_data_reset (modify_task_data);
        set_client_state (CLIENT_AUTHENTIC);
        break;
      ...
      case CLIENT_MODIFY_TASK_PREFERENCES_PREFERENCE:
        array_add (modify_task_data->preferences, modify_task_data->preference);
      ...
      case CLIENT_MODIFY_TLS_CERTIFICATE:
        if (modify_tls_certificate_element_end (gmp_parser, error,
                                                element_name))
          set_client_state (CLIENT_AUTHENTIC);
        break;
      ...
      case CLIENT_MODIFY_USER:
        {
          if ((modify_user_data->name == NULL
            ...
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "modify_user", "A NAME or user_id is required"));
          else
            {
              ...
              switch (modify_user (
                ...
                ((modify_user_data->modify_password
                  && modify_user_data->password)
                   ...
                   : NULL),
                modify_user_data->comment, modify_user_data->hosts,
                modify_user_data->hosts_allow, modify_user_data->ifaces,
                modify_user_data->ifaces_allow, modify_user_data->sources,
                modify_user_data->groups, &fail_group_id,
                modify_user_data->roles, &fail_role_id, &errdesc))
                {
                case 0:
                  SEND_TO_CLIENT_OR_FAIL (XML_OK ("modify_user"));
                  break;
                case 1:
                  if (send_find_error_to_client ("modify_user", "group",
                                                 fail_group_id, gmp_parser))
                    {
                      error_send_to_client (error);
                      return;
                  ...
                  break;
                case 2:
                  if (send_find_error_to_client ("modify_user", "user",
                                                 modify_user_data->user_id
                                                   ?: modify_user_data->name,
                                                 gmp_parser))
                    {
                ...
                case 4:
                  ...
                  break;
                case 5:
                  if (send_find_error_to_client ("modify_user", "role",
                                                 fail_role_id, gmp_parser))
                    {
                      error_send_to_client (error);
                      return;
                ...
                case 6:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_user", "Error in host specification"));
                  log_event_fail ("user", "User", NULL, "modified");
                  break;
                case 7:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_user", "Error in user name"));
                  log_event_fail ("user", "User", NULL, "modified");
                  break;
                case 8:
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "modify_user", "User with name exists already"));
                  log_event_fail ("user", "User", NULL, "modified");
                  break;
                case 99:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_user", "Permission denied"));
                  break;
                case -2:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_user", "Unknown role"));
                  break;
                case -3:
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("modify_user", "Error in SOURCES"));
                  break;
                case -1:
                  if (errdesc)
                    {
                      char *buf =
                        make_xml_error_syntax ("modify_user", errdesc);
                      SEND_TO_CLIENT_OR_FAIL (buf);
                      g_free (buf);
                      break;
                    }
      ...
      case CLIENT_MOVE_TASK:
        if (move_task_data->task_id == NULL
          ...
          {
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("move_task", "A non-empty task_id"
                                             " attribute is required"));
            break;
          }
        ...
        if (move_task_data->slave_id == NULL)
          {
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "move_task", "A slave_id attribute is required"));
            break;
          }

        switch (move_task (move_task_data->task_id, move_task_data->slave_id))
          {
          case 0:
            SEND_TO_CLIENT_OR_FAIL (XML_OK ("move_task"));
            break;
          case 2:
            if (send_find_error_to_client ("move_task", "Task",
                                           move_task_data->task_id, gmp_parser))
              {
                error_send_to_client (error);
                return;
              }
          ...
          case 3:
            if (send_find_error_to_client (
                  "move_task", "Slave", move_task_data->slave_id, gmp_parser))
              {
                error_send_to_client (error);
                return;
              }
          ...
          case 4:
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "move_task", "Task must use an OpenVAS scanner to assign"
                           " a slave."));
            break;
          case 5:
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "move_task", "Task cannot be stopped at the moment."));
            break;
          case 6:
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("move_task", "Scanner does not allow stopping"
                                             " the Task."));
            break;
          case 7:
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "move_task", "Destination scanner does not support"
                           " slaves."));
            break;
          case 98:
            SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              "move_task", "Permission to stop and resume denied"));
            break;
          case 99:
            SEND_TO_CLIENT_OR_FAIL (
              XML_ERROR_SYNTAX ("move_task", "Permission denied"));
            break;
          default: /* Programming error. */
            SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("move_task"));
            assert (0);
            break;
          }
        ...
        if (test_alert_data->alert_id)
          {
            ...
            switch (
              manage_test_alert (test_alert_data->alert_id, &script_message))
              {
              case 0:
                SEND_TO_CLIENT_OR_FAIL (XML_OK ("test_alert"));
                break;
              case 1:
                if (send_find_error_to_client ("test_alert", "alert",
                                               test_alert_data->alert_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("test_alert", "Permission denied"));
                break;
              case 2:
              case -1:
              ...
              case -2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "test_alert", "Failed to find report format for"
                                " alert"));
                break;
              case -3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "test_alert", "Failed to find filter for alert"));
                break;
              case -4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "test_alert", "Failed to find credential for alert"));
                break;
              case -5:
                if (script_message)
                  {
                  ...
                  {
                    SEND_TO_CLIENT_OR_FAIL (
                      XML_ERROR_SYNTAX ("test_alert", "Alert script failed"));
                  }
                break;
              default: /* Programming error. */
                assert (0);
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("test_alert",
                                                    "An alert_id"
                                                    " attribute is required"));
        ...
        if (restore_data->id)
          {
            switch (manage_restore (restore_data->id))
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "restore", "Resource refers into trashcan"));
                break;
              case 2:
              ...
              case 3:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "restore", "A resource with this name exists"
                             " already"));
                break;
              case 4:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "restore", "A resource with this UUID exists"
                             " already"));
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("restore", "Permission denied"));
                break;
              default: /* Programming error. */
                assert (0);
              case -1:
          ...
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("restore", "An id attribute is required"));
        ...
        if (resume_task_data->task_id)
          {
            ...
            switch (resume_task (resume_task_data->task_id, &report_id))
              ...
              case 0:
                {
                  ...
                  msg = g_strdup_printf (
                    ...
                    report_id);
                  free (report_id);
                  if (send_to_client (msg, write_to_client,
                                      write_to_client_data))
                    {
                      g_free (msg);
                      error_send_to_client (error);
                ...
                log_event ("task", "Task", resume_task_data->task_id,
                           "resumed");
                break;
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("resume_task", "Task is active already"));
                log_event_fail ("task", "Task", resume_task_data->task_id,
                                "resumed");
                break;
              case 22:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
              ...
              case 4:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("resume_task", "Resuming not supported"));
                log_event_fail ("task", "Task", resume_task_data->task_id,
                                "resumed");
                break;
              case 3: /* Find failed. */
                if (send_find_error_to_client ("resume_task", "task",
                                               resume_task_data->task_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("resume_task", "Permission denied"));
                log_event_fail ("task", "Task", resume_task_data->task_id,
                                "resumed");
                break;
              case -6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "resume_task", "There is already a task running in"
                                 " this process"));
                log_event_fail ("task", "Task", resume_task_data->task_id,
                                "resumed");
                break;
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("resume_task",
                                                    "A task_id"
                                                    " attribute is required"));
        ...
        if (run_wizard_data->name)
          {
            ...
            switch (manage_run_wizard (
              run_wizard_data->name,
              (int (*) (void *, gchar *, gchar **)) process_gmp, gmp_parser,
              run_wizard_data->params, read_only, run_wizard_data->mode,
              &command_error, &command_error_code, &response))
              {
              case 0:
                {
                  gchar *msg;
                  msg = g_strdup_printf (
                    ...
                    command_error_code ? command_error_code
                                       : STATUS_OK_REQUESTED,
                    command_error ? command_error : STATUS_OK_REQUESTED_TEXT,
              ...
              case 1:
                {
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "run_wizard", "NAME characters must be alphanumeric"
                                  " or underscore"));
              ...
              case 5:
                {
                  SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                    "run_wizard", "Wizard is not marked as read only"));
                  break;
                }
              ...
              case 99:
                {
                  SEND_TO_CLIENT_OR_FAIL (
                    XML_ERROR_SYNTAX ("run_wizard", "Permission denied"));
                  break;
                }
        ...
        if (start_task_data->task_id)
          {
            ...
            switch (start_task (start_task_data->task_id, &report_id))
              ...
              case 1:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("start_task", "Task is active already"));
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("start_task", "Permission denied"));
                log_event_fail ("task", "Task", start_task_data->task_id,
                                "started");
                break;
              case -2:
                 ...
                 * tasks. */
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("start_task", "Task must have a target"));
                log_event_fail ("task", "Task", start_task_data->task_id,
                                "started");
                break;
              case -4:
              ...
              case -6:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "start_task", "There is already a task running in"
                                " this process"));
                log_event_fail ("task", "Task", start_task_data->task_id,
                                "started");
                break;
          ...
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("start_task", "A task_id attribute is required"));
        ...
        if (stop_task_data->task_id)
          {
            switch (stop_task (stop_task_data->task_id))
              ...
              case 0: /* Stopped. */
                ...
                log_event ("task", "Task", stop_task_data->task_id, "stopped");
                break;
              ...
              case 3: /* Find failed. */
                if (send_find_error_to_client (
                      "stop_task", "task", stop_task_data->task_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                  XML_ERROR_SYNTAX ("stop_task", "Permission denied"));
          ...
          SEND_TO_CLIENT_OR_FAIL (
            XML_ERROR_SYNTAX ("stop_task", "A task_id attribute is required"));
      ...
      case CLIENT_VERIFY_REPORT_FORMAT:
        if (verify_report_format_data->report_format_id)
            ...
            switch (verify_report_format (
              ...
              case 1:
                if (send_find_error_to_client (
                      ...
                      verify_report_format_data->report_format_id, gmp_parser))
                  {
                    error_send_to_client (error);
                    return;
                  }
              ...
              case 99:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
                  "verify_report_format", "Permission denied"));
                break;
              default:
                SEND_TO_CLIENT_OR_FAIL (
          ...
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX ("verify_report_format",
                                                    "A report_format_id"
                                                    " attribute is required"));
        ...
        if (verify_scanner_data->scanner_id)
          {
            ...
            switch (verify_scanner (verify_scanner_data->scanner_id, &version))
              ...
              case 0:
                ...
                break;
              case 1:
                if (send_find_error_to_client ("verify_scanner", "scanner",
                                               verify_scanner_data->scanner_id,
                                               gmp_parser))
                  {
                    error_send_to_client (error);
              ...
              case 2:
                SEND_TO_CLIENT_OR_FAIL (XML_ERROR_UNAVAILABLE (
                  "verify_scanner", "Service unavailable"));
                break;
              case 3:
                SENDF_TO_CLIENT_OR_FAIL (
                  ...
                  "<version>%s</version>"
                  "</verify_scanner_response>",
                  STATUS_SERVICE_UNAVAILABLE, version);
                break;
              case 99:
                SEND_TO_CLIENT_OR_FAIL (
                ...
                break;
              default:
                SEND_TO_CLIENT_OR_FAIL (XML_INTERNAL_ERROR ("verify_scanner"));
                break;
              }
          }
        else
          SEND_TO_CLIENT_OR_FAIL (XML_ERROR_SYNTAX (
            "verify_scanner", "A scanner_id attribute is required"));
                     ...
                     /* unused */ GError **error)
{
  ...
  switch (client_state)
    ...
    case CLIENT_AUTHENTICATE_CREDENTIALS_PASSWORD:
      ...
      APPEND (CLIENT_MODIFY_CREDENTIAL_LOGIN, &modify_credential_data->login);

      APPEND (CLIENT_MODIFY_CREDENTIAL_NAME, &modify_credential_data->name);
      ...
      APPEND (CLIENT_MODIFY_CONFIG_COMMENT, &modify_config_data->comment);

      APPEND (CLIENT_MODIFY_CONFIG_SCANNER, &modify_config_data->scanner_id);

      APPEND (CLIENT_MODIFY_CONFIG_NAME, &modify_config_data->name);
      ...
      APPEND (CLIENT_MODIFY_SETTING_NAME, &modify_setting_data->name);

      APPEND (CLIENT_MODIFY_SETTING_VALUE, &modify_setting_data->value);

      APPEND (CLIENT_MODIFY_TASK_ALTERABLE, &modify_task_data->alterable);

      APPEND (CLIENT_MODIFY_TASK_COMMENT, &modify_task_data->comment);

      APPEND (CLIENT_MODIFY_TASK_HOSTS_ORDERING,
              &modify_task_data->hosts_ordering);

      APPEND (CLIENT_MODIFY_TASK_NAME, &modify_task_data->name);

      APPEND (CLIENT_MODIFY_TASK_OBSERVERS, &modify_task_data->observers);

      APPEND (CLIENT_MODIFY_TASK_FILE, &modify_task_data->file);
      ...
      APPEND (CLIENT_MODIFY_USER_COMMENT, &modify_user_data->comment);

      APPEND (CLIENT_MODIFY_USER_HOSTS, &modify_user_data->hosts);

      APPEND (CLIENT_MODIFY_USER_IFACES, &modify_user_data->ifaces);

      APPEND (CLIENT_MODIFY_USER_NAME, &modify_user_data->name);

      APPEND (CLIENT_MODIFY_USER_NEW_NAME, &modify_user_data->new_name);

      APPEND (CLIENT_MODIFY_USER_PASSWORD, &modify_user_data->password);

      APPEND (CLIENT_MODIFY_USER_SOURCES_SOURCE,
              &modify_user_data->current_source);

      APPEND (CLIENT_CREATE_ASSET_ASSET_COMMENT, &create_asset_data->comment);

      APPEND (CLIENT_CREATE_ASSET_ASSET_NAME, &create_asset_data->name);

      APPEND (CLIENT_CREATE_ASSET_ASSET_TYPE, &create_asset_data->type);
      ...
      APPEND (CLIENT_CREATE_CREDENTIAL_COPY, &create_credential_data->copy);
      ...
      APPEND (CLIENT_CREATE_CREDENTIAL_LOGIN, &create_credential_data->login);

      APPEND (CLIENT_CREATE_CREDENTIAL_NAME, &create_credential_data->name);
      ...
      APPEND (CLIENT_CREATE_CREDENTIAL_TYPE, &create_credential_data->type);

      APPEND (CLIENT_CREATE_ALERT_ACTIVE, &create_alert_data->active);

      APPEND (CLIENT_CREATE_ALERT_COMMENT, &create_alert_data->comment);

      APPEND (CLIENT_CREATE_ALERT_COPY, &create_alert_data->copy);

      APPEND (CLIENT_CREATE_ALERT_CONDITION, &create_alert_data->condition);

      APPEND (CLIENT_CREATE_ALERT_EVENT, &create_alert_data->event);

      APPEND (CLIENT_CREATE_ALERT_METHOD, &create_alert_data->method);

      APPEND (CLIENT_CREATE_ALERT_NAME, &create_alert_data->name);

      APPEND (CLIENT_CREATE_ALERT_CONDITION_DATA,
              &create_alert_data->part_data);

      APPEND (CLIENT_CREATE_ALERT_EVENT_DATA, &create_alert_data->part_data);

      APPEND (CLIENT_CREATE_ALERT_METHOD_DATA, &create_alert_data->part_data);
    ...
    case CLIENT_CREATE_CONFIG:
      ...
      APPEND (CLIENT_CREATE_FILTER_COMMENT, &create_filter_data->comment);

      APPEND (CLIENT_CREATE_FILTER_COPY, &create_filter_data->copy);

      APPEND (CLIENT_CREATE_FILTER_NAME, &create_filter_data->name);

      APPEND (CLIENT_CREATE_FILTER_TERM, &create_filter_data->term);

      APPEND (CLIENT_CREATE_FILTER_TYPE, &create_filter_data->type);

      APPEND (CLIENT_CREATE_GROUP_COMMENT, &create_group_data->comment);

      APPEND (CLIENT_CREATE_GROUP_COPY, &create_group_data->copy);

      APPEND (CLIENT_CREATE_GROUP_NAME, &create_group_data->name);

      APPEND (CLIENT_CREATE_GROUP_USERS, &create_group_data->users);

      APPEND (CLIENT_CREATE_NOTE_ACTIVE, &create_note_data->active);

      APPEND (CLIENT_CREATE_NOTE_COPY, &create_note_data->copy);

      APPEND (CLIENT_CREATE_NOTE_HOSTS, &create_note_data->hosts);

      APPEND (CLIENT_CREATE_NOTE_PORT, &create_note_data->port);

      APPEND (CLIENT_CREATE_NOTE_SEVERITY, &create_note_data->severity);

      APPEND (CLIENT_CREATE_NOTE_TEXT, &create_note_data->text);

      APPEND (CLIENT_CREATE_NOTE_THREAT, &create_note_data->threat);

      APPEND (CLIENT_CREATE_OVERRIDE_ACTIVE, &create_override_data->active);

      APPEND (CLIENT_CREATE_OVERRIDE_COPY, &create_override_data->copy);

      APPEND (CLIENT_CREATE_OVERRIDE_HOSTS, &create_override_data->hosts);
      ...
      APPEND (CLIENT_CREATE_OVERRIDE_PORT, &create_override_data->port);

      APPEND (CLIENT_CREATE_OVERRIDE_SEVERITY, &create_override_data->severity);

      APPEND (CLIENT_CREATE_OVERRIDE_TEXT, &create_override_data->text);

      APPEND (CLIENT_CREATE_OVERRIDE_THREAT, &create_override_data->threat);

      APPEND (CLIENT_CREATE_PERMISSION_COMMENT,
              &create_permission_data->comment);

      APPEND (CLIENT_CREATE_PERMISSION_COPY, &create_permission_data->copy);

      APPEND (CLIENT_CREATE_PERMISSION_NAME, &create_permission_data->name);
    ...
    case CLIENT_CREATE_PORT_LIST:
      ...
      APPEND (CLIENT_CREATE_PORT_RANGE_END, &create_port_range_data->end);

      APPEND (CLIENT_CREATE_PORT_RANGE_START, &create_port_range_data->start);

      APPEND (CLIENT_CREATE_PORT_RANGE_TYPE, &create_port_range_data->type);

      APPEND (CLIENT_CREATE_REPORT_IN_ASSETS, &create_report_data->in_assets);
      ...
      APPEND (CLIENT_CREATE_REPORT_RR_HOST_END, &create_report_data->host_end);
      ...
      APPEND (CLIENT_CREATE_REPORT_RR_SCAN_END, &create_report_data->scan_end);
    ...
    case CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_DETECTION_RESULT_DETAILS_DETAIL_NAME:
      gvm_append_text (&create_report_data->result_detection_name, text,
                       text_len);
      break;
    case CLIENT_CREATE_REPORT_RR_RESULTS_RESULT_DETECTION_RESULT_DETAILS_DETAIL_VALUE:
      if (create_report_data->result_detection_name != NULL)
        {
          if (strcmp ("product", create_report_data->result_detection_name)
              == 0)
            {
              gvm_append_text (&create_report_data->result_detection_product,
                               text, text_len);
            }
                   ...
                   == 0)
            {
              gvm_append_text (&create_report_data->result_detection_location,
                               text, text_len);
            }
          else if (strcmp ("source_oid",
                           create_report_data->result_detection_name)
                   == 0)
            {
              gvm_append_text (&create_report_data->result_detection_source_oid,
                               text, text_len);
            }
          else if (strcmp ("source_name",
                           create_report_data->result_detection_name)
                   == 0)
            {
              gvm_append_text (
                &create_report_data->result_detection_source_name, text,
                text_len);
            }
      ...
      APPEND (CLIENT_CREATE_REPORT_RR_H_END, &create_report_data->host_end);

      APPEND (CLIENT_CREATE_REPORT_RR_H_IP, &create_report_data->ip);

      APPEND (CLIENT_CREATE_REPORT_RR_H_START, &create_report_data->host_start);

    case CLIENT_CREATE_REPORT_FORMAT:
      create_report_format_element_text (text, text_len);
      break;

      APPEND (CLIENT_CREATE_ROLE_COMMENT, &create_role_data->comment);

      APPEND (CLIENT_CREATE_ROLE_COPY, &create_role_data->copy);

      APPEND (CLIENT_CREATE_ROLE_NAME, &create_role_data->name);

      APPEND (CLIENT_CREATE_ROLE_USERS, &create_role_data->users);

      APPEND (CLIENT_CREATE_SCANNER_NAME, &create_scanner_data->name);

      APPEND (CLIENT_CREATE_SCANNER_COMMENT, &create_scanner_data->comment);

      APPEND (CLIENT_CREATE_SCANNER_COPY, &create_scanner_data->copy);

      APPEND (CLIENT_CREATE_SCANNER_HOST, &create_scanner_data->host);

      APPEND (CLIENT_CREATE_SCANNER_PORT, &create_scanner_data->port);

      APPEND (CLIENT_CREATE_SCANNER_TYPE, &create_scanner_data->type);

      APPEND (CLIENT_CREATE_SCANNER_CA_PUB, &create_scanner_data->ca_pub);

      APPEND (CLIENT_CREATE_SCHEDULE_COMMENT, &create_schedule_data->comment);

      APPEND (CLIENT_CREATE_SCHEDULE_COPY, &create_schedule_data->copy);

      APPEND (CLIENT_CREATE_SCHEDULE_ICALENDAR,
              &create_schedule_data->icalendar);

      APPEND (CLIENT_CREATE_SCHEDULE_NAME, &create_schedule_data->name);

      APPEND (CLIENT_CREATE_SCHEDULE_TIMEZONE, &create_schedule_data->timezone);

      APPEND (CLIENT_CREATE_TAG_ACTIVE, &create_tag_data->active);

      APPEND (CLIENT_CREATE_TAG_RESOURCES_TYPE,
              &create_tag_data->resource_type);

      APPEND (CLIENT_CREATE_TAG_COPY, &create_tag_data->copy);

      APPEND (CLIENT_CREATE_TAG_COMMENT, &create_tag_data->comment);

      APPEND (CLIENT_CREATE_TAG_NAME, &create_tag_data->name);

      APPEND (CLIENT_CREATE_TAG_VALUE, &create_tag_data->value);
      ...
      APPEND (CLIENT_CREATE_TARGET_COMMENT, &create_target_data->comment);

      APPEND (CLIENT_CREATE_TARGET_COPY, &create_target_data->copy);

      APPEND (CLIENT_CREATE_TARGET_HOSTS, &create_target_data->hosts);

      APPEND (CLIENT_CREATE_TARGET_NAME, &create_target_data->name);

      APPEND (CLIENT_CREATE_TARGET_PORT_RANGE, &create_target_data->port_range);
      ...
      APPEND (CLIENT_CREATE_TASK_ALTERABLE, &create_task_data->alterable);
    ...
    case CLIENT_CREATE_TASK_COMMENT:
      ...
      APPEND (CLIENT_CREATE_TASK_COPY, &create_task_data->copy);

      APPEND (CLIENT_CREATE_TASK_NAME, &create_task_data->name);

      APPEND (CLIENT_CREATE_TASK_OBSERVERS, &create_task_data->observers);
      ...
      APPEND (CLIENT_CREATE_TASK_USAGE_TYPE, &create_task_data->usage_type);
    ...
    case CLIENT_CREATE_TLS_CERTIFICATE:
      ...
      APPEND (CLIENT_CREATE_USER_COMMENT, &create_user_data->comment);

      APPEND (CLIENT_CREATE_USER_COPY, &create_user_data->copy);

      APPEND (CLIENT_CREATE_USER_HOSTS, &create_user_data->hosts);

      APPEND (CLIENT_CREATE_USER_IFACES, &create_user_data->ifaces);

      APPEND (CLIENT_CREATE_USER_NAME, &create_user_data->name);

      APPEND (CLIENT_CREATE_USER_PASSWORD, &create_user_data->password);
    ...
    case CLIENT_GET_AGGREGATES_TEXT_COLUMN:
      ...
      APPEND (CLIENT_MODIFY_ALERT_NAME, &modify_alert_data->name);

      APPEND (CLIENT_MODIFY_ALERT_COMMENT, &modify_alert_data->comment);

      APPEND (CLIENT_MODIFY_ALERT_ACTIVE, &modify_alert_data->active);

      APPEND (CLIENT_MODIFY_ALERT_EVENT, &modify_alert_data->event);

      APPEND (CLIENT_MODIFY_ALERT_CONDITION, &modify_alert_data->condition);

      APPEND (CLIENT_MODIFY_ALERT_METHOD, &modify_alert_data->method);

      APPEND (CLIENT_MODIFY_ALERT_EVENT_DATA, &modify_alert_data->part_data);

      APPEND (CLIENT_MODIFY_ALERT_CONDITION_DATA,
              &modify_alert_data->part_data);

      APPEND (CLIENT_MODIFY_ALERT_METHOD_DATA, &modify_alert_data->part_data);
      ...
      APPEND (CLIENT_MODIFY_ASSET_COMMENT, &modify_asset_data->comment);
      ...
      APPEND (CLIENT_MODIFY_FILTER_COMMENT, &modify_filter_data->comment);

      APPEND (CLIENT_MODIFY_FILTER_NAME, &modify_filter_data->name);

      APPEND (CLIENT_MODIFY_FILTER_TERM, &modify_filter_data->term);

      APPEND (CLIENT_MODIFY_FILTER_TYPE, &modify_filter_data->type);

      APPEND (CLIENT_MODIFY_GROUP_COMMENT, &modify_group_data->comment);

      APPEND (CLIENT_MODIFY_GROUP_NAME, &modify_group_data->name);

      APPEND (CLIENT_MODIFY_GROUP_USERS, &modify_group_data->users);

      APPEND (CLIENT_MODIFY_NOTE_ACTIVE, &modify_note_data->active);

      APPEND (CLIENT_MODIFY_NOTE_HOSTS, &modify_note_data->hosts);

      APPEND (CLIENT_MODIFY_NOTE_PORT, &modify_note_data->port);

      APPEND (CLIENT_MODIFY_NOTE_SEVERITY, &modify_note_data->severity);

      APPEND (CLIENT_MODIFY_NOTE_TEXT, &modify_note_data->text);

      APPEND (CLIENT_MODIFY_NOTE_THREAT, &modify_note_data->threat);

      APPEND (CLIENT_MODIFY_NOTE_NVT, &modify_note_data->nvt_oid);

      APPEND (CLIENT_MODIFY_OVERRIDE_ACTIVE, &modify_override_data->active);

      APPEND (CLIENT_MODIFY_OVERRIDE_HOSTS, &modify_override_data->hosts);
      ...
      APPEND (CLIENT_MODIFY_OVERRIDE_PORT, &modify_override_data->port);

      APPEND (CLIENT_MODIFY_OVERRIDE_SEVERITY, &modify_override_data->severity);

      APPEND (CLIENT_MODIFY_OVERRIDE_TEXT, &modify_override_data->text);

      APPEND (CLIENT_MODIFY_OVERRIDE_THREAT, &modify_override_data->threat);

      APPEND (CLIENT_MODIFY_PERMISSION_COMMENT,
              &modify_permission_data->comment);

      APPEND (CLIENT_MODIFY_PERMISSION_NAME, &modify_permission_data->name);
      ...
      APPEND (CLIENT_MODIFY_PORT_LIST_COMMENT, &modify_port_list_data->comment);

      APPEND (CLIENT_MODIFY_PORT_LIST_NAME, &modify_port_list_data->name);

      APPEND (CLIENT_MODIFY_ROLE_COMMENT, &modify_role_data->comment);

      APPEND (CLIENT_MODIFY_ROLE_NAME, &modify_role_data->name);

      APPEND (CLIENT_MODIFY_ROLE_USERS, &modify_role_data->users);

      APPEND (CLIENT_MODIFY_SCANNER_COMMENT, &modify_scanner_data->comment);

      APPEND (CLIENT_MODIFY_SCANNER_NAME, &modify_scanner_data->name);

      APPEND (CLIENT_MODIFY_SCANNER_HOST, &modify_scanner_data->host);

      APPEND (CLIENT_MODIFY_SCANNER_PORT, &modify_scanner_data->port);

      APPEND (CLIENT_MODIFY_SCANNER_TYPE, &modify_scanner_data->type);

      APPEND (CLIENT_MODIFY_SCANNER_CA_PUB, &modify_scanner_data->ca_pub);

      APPEND (CLIENT_MODIFY_SCHEDULE_COMMENT, &modify_schedule_data->comment);

      APPEND (CLIENT_MODIFY_SCHEDULE_ICALENDAR,
              &modify_schedule_data->icalendar);

      APPEND (CLIENT_MODIFY_SCHEDULE_NAME, &modify_schedule_data->name);

      APPEND (CLIENT_MODIFY_SCHEDULE_TIMEZONE, &modify_schedule_data->timezone);

      APPEND (CLIENT_MODIFY_TAG_ACTIVE, &modify_tag_data->active);

      APPEND (CLIENT_MODIFY_TAG_RESOURCES_TYPE,
              &modify_tag_data->resource_type);

      APPEND (CLIENT_MODIFY_TAG_COMMENT, &modify_tag_data->comment);

      APPEND (CLIENT_MODIFY_TAG_NAME, &modify_tag_data->name);

      APPEND (CLIENT_MODIFY_TAG_VALUE, &modify_tag_data->value);
      ...
      APPEND (CLIENT_MODIFY_TARGET_COMMENT, &modify_target_data->comment);

      APPEND (CLIENT_MODIFY_TARGET_HOSTS, &modify_target_data->hosts);

      APPEND (CLIENT_MODIFY_TARGET_NAME, &modify_target_data->name);
    ...
    case CLIENT_MODIFY_TLS_CERTIFICATE:
      ...
      APPEND (CLIENT_RUN_WIZARD_MODE, &run_wizard_data->mode);

      APPEND (CLIENT_RUN_WIZARD_NAME, &run_wizard_data->name);
 ...
 */
static void
gmp_xml_handle_error (/* unused */ GMarkupParseContext *context, GError *error,
                      /* unused */ gpointer user_data)
{
...
{
  g_log_set_handler (G_LOG_DOMAIN, ALL_LOG_LEVELS, (GLogFunc) gvm_log_func,
                     log_config);
                  ...
                  void *write_to_client_data, gchar **disable)
{
  ...
  xml_context = g_markup_parse_context_new (
    &xml_parser, 0,
    gmp_parser_new (write_to_client, write_to_client_data, disable),
  ...
  if (success == FALSE)
    {
      ...
      if (error)
        {
          err = -4;
          if (g_error_matches (error, G_MARKUP_ERROR,
                               G_MARKUP_ERROR_UNKNOWN_ELEMENT))
            g_debug ("   client error: G_MARKUP_ERROR_UNKNOWN_ELEMENT");
          else if (g_error_matches (error, G_MARKUP_ERROR,
...
process_gmp (gmp_parser_t *parser, const gchar *command, gchar **response)
{
  ...
  success = g_markup_parse_context_parse (xml_context, command,
