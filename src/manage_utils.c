...
hosts_str_contains (const char* hosts_str, const char* find_host_str,
                    int max_hosts)
{
  openvas_hosts_t *hosts, *find_hosts;

  hosts = openvas_hosts_new_with_max (hosts_str, max_hosts);
  find_hosts = openvas_hosts_new_with_max (find_host_str, 1);

  if (hosts == NULL || find_hosts == NULL || find_hosts->count != 1)
    {
      openvas_hosts_free (hosts);
      openvas_hosts_free (find_hosts);
      return 0;
    }

  int ret = openvas_host_in_hosts (find_hosts->hosts->data, NULL, hosts);
  openvas_hosts_free (hosts);
  openvas_hosts_free (find_hosts);
  return ret;
}
