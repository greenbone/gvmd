...
/**
 * @file
 * @brief GVM management layer: Delta audit report exports.
 */
...
/**
 * @brief Validate and resolve resources used by a delta audit report export.
 *
...
validate_delta_audit_report_export (
  ...
  report_config_t *report_config)
{
  ...
  *report_config = 0;
  ...
  if (*delta_report == 0)
    return MANAGE_EXPORT_AUDIT_DELTA_REPORT_NOT_FOUND;
...
/**
 * @brief Create a queued delta audit report export.
 *
 * @param[in]  report_id          UUID of the report to export.
 ...
 *
 * @return Result of creating the delta audit report export.
 */
manage_export_audit_report_response_t
manage_export_delta_audit_report (
  const gchar *report_id,
  const gchar *delta_report_id,
  const gchar *report_format_id,
  const gchar *report_config_id,
  ...
  gboolean *created)
{
  ...
  *created = FALSE;
  ...
  response = validate_delta_audit_report_export (
    report_id,
    &report,
    delta_report_id,
    &delta_report,
...
/**
 * @brief Generate the intermediate XML used by a delta audit report format.
 *
 ...
 */
static int
generate_delta_audit_report_export_xml (
...
/**
 * @brief Process a queued delta audit report export.
 *
 ...
 */
int
manage_process_delta_audit_report_export (report_export_t report_export)
{
  ...
  if (data->export_type != REPORT_EXPORT_TYPE_DELTA_AUDIT
      || data->delta_report == 0)
    {
      manage_fail_report_export (
        report_export,
        "Unsupported delta audit report export type");
      goto cleanup;
    }
  ...
  ret = generate_delta_audit_report_export_xml (
    data,
  ...
  if (ret)
    {
      manage_fail_report_export (
        ...
        ret == 2
          ...
          : "Failed to generate delta audit report XML");
