...
/**
 * @file
 * @brief GVM management layer: Delta scan report exports.
 */
...
/**
 * @brief Validate and resolve resources used by a delta scan report export.
 *
 * @param[in]  report_id          UUID of the report to export.
 * @param[out] report             Resolved report.
 * @param[in]  delta_report_id    UUID of the delta report.
 * @param[out] delta_report       Resolved delta report.
...
validate_delta_scan_report_export (
  const gchar *report_id,
  report_t *report,
  const gchar *delta_report_id,
  ...
  report_config_t *report_config)
{
  ...
  if (str_blank (report_id) == TRUE
      || report == NULL
      || str_blank (delta_report_id) == TRUE
      || delta_report == NULL
  ...
  *report_config = 0;
  ...
  /*
   * Resolve delta report.
   */
  ret = find_report_with_permission (delta_report_id,
                                     delta_report,
                                     "get_reports");
  if (ret)
    return MANAGE_EXPORT_SCAN_REPORT_ERROR;
  ...
  if (*delta_report == 0)
    return MANAGE_EXPORT_SCAN_DELTA_REPORT_NOT_FOUND;

  /*
   * A report cannot be compared with itself.
   */
  if (*report == *delta_report)
    return MANAGE_EXPORT_SCAN_REPORT_INVALID_DELTA_REPORT;
  ...
  if (report_task (*delta_report, &delta_task))
    return MANAGE_EXPORT_SCAN_REPORT_ERROR;
  ...
  /*
   * Both reports must have the same usage type and must be scan reports.
   */
  usage_type = NULL;
  ...
  /*
   * Resolve optional report config.
   */
...
/**
 * @brief Create a queued delta scan report export.
 *
 * @param[in]  report_id          UUID of the report to export.
 * @param[in]  delta_report_id    UUID of the report used for comparison.
 * @param[in]  report_format_id   UUID of the report format.
 * @param[in]  report_config_id   Optional UUID of the report config.
 ...
 * @param[out] report_export      Created or reused report export.
 * @param[out] status             Current status of the report export.
 * @param[out] created            TRUE if a new report export was created.
 *
 * @return Result of creating the delta scan report export.
 */
manage_export_scan_report_response_t
manage_export_delta_scan_report (
  const gchar *report_id,
  const gchar *delta_report_id,
  const gchar *report_format_id,
  const gchar *report_config_id,
  ...
  gboolean *created)
{
  ...
  *created = FALSE;
  ...
  response = validate_delta_scan_report_export (
    report_id,
    &report,
    delta_report_id,
    &delta_report,
  ...
  if (manage_create_report_export (
        report,
        delta_report,
        report_format,
        report_config,
...
/**
 * @brief Generate the intermediate XML used by a delta report format.
 *
 ...
 */
static int
generate_delta_scan_report_export_xml (
  ...
  gchar *xml_path)
{
  ...
  ret = manage_print_delta_report_xml_start (
    data->report,
    data->delta_report,
    task,
    xml_path,
...
/**
 * @brief Process a queued delta scan report export.
 *
 ...
 */
int
manage_process_delta_scan_report_export (report_export_t report_export)
{
  ...
  if (data->export_type != REPORT_EXPORT_TYPE_DELTA_SCAN
      || data->delta_report == 0)
    {
      manage_fail_report_export (
        report_export,
        "Unsupported delta scan report export type");
      goto cleanup;
    }
  ...
  ret = generate_delta_scan_report_export_xml (
    data,
  ...
  if (ret)
    {
      manage_fail_report_export (
        ...
        ret == 2
          ...
          : "Failed to generate delta scan report XML");
